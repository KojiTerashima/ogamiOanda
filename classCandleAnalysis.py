...
class candleAnalysis:
    ...
    def __init__(self, base_oa, target_time_jp, notifier: Notifier | None = None):
        ...
        if cache.latest_df_d5_df_r is None:
            ...
            if same:
                print(
                    "同じデータのたpeaks_class_30mめ、データ新規取得＆Peaks生成は呼ばず(主にcandleLCChangeで発生)  既存:",
                    t1,
                    ",現時刻:",
                    t2,
                )
        ...
        if self.d5_df_r is None:
            ...
            print(
                ...
                self.d5_df_r.iloc[0]["time_jp"],
                "to",
                self.d5_df_r.iloc[-1]["time_jp"],
            )
        ...
        self.peaks_class = peaksClass.PeaksClass(
            self.d5_df_r, granularity
        )  # ★★peaks_classインスタンスの生成
    ...
    def update_s5_df(self, target_time_jp=0):
        if target_time_jp == 0:
            # nowでやる場合（通常）
            # 5秒足で
            s5_df_res = self.base_oa.InstrumentsCandles_multi_exe(
                "USD_JPY", {"granularity": "S5", "count": 5}, 1
            )  # 時間昇順(直近が最後尾）
            ...
            self.s5_df_r = s5_df_latest_bottom.sort_index(
                ascending=False
            )  # 直近が上の方にある＝時間降順に変更
    ...
    def get_date_df(self, target_time_jp):
        ...
        if target_time_jp == 0:
            # nowでやる場合（通常）
            # 5分足のデータ
            d5_df_res = self.base_oa.InstrumentsCandles_multi_exe(
                "USD_JPY", {"granularity": "M5", "count": self.need_df_num}, 1
            )  # 時間昇順(直近が最後尾）
            ...
            self.d5_df_r = d5_df_latest_bottom.sort_index(
                ascending=False
            )  # 直近が上の方にある＝時間降順に変更

            # 60分足のデータ
            d60_df_res = self.base_oa.InstrumentsCandles_multi_exe(
                "USD_JPY", {"granularity": "H1", "count": self.need_df_num}, 1
            )  # 時間昇順(直近が最後尾）
            ...
            self.d60_df_r = d60_df_latest_bottom.sort_index(
                ascending=False
            )  # 直近が上の方にある＝時間降順に変更
            ...
            # 30分足のデータ
            d30_df_res = self.base_oa.InstrumentsCandles_multi_exe(
                "USD_JPY", {"granularity": "M30", "count": self.need_df_num}, 1
            )  # 時間昇順(直近が最後尾）
            ...
            self.d30_df_r = m30_df_latest_bottom.sort_index(
                ascending=False
            )  # 直近が上の方にある＝時間降順に変更
            ...
            param = {
                "granularity": "M5",
                "count": self.need_df_num,
                "to": euro_time_datetime_iso,
            }
            d5_df_res = self.base_oa.InstrumentsCandles_exe(
                "USD_JPY", param
            )  # 時間昇順(直近が最後尾）
            if d5_df_res["error"] == -1:
                print("error Candle")
            ...
            d60_df_res = self.base_oa.InstrumentsCandles_exe(
                "USD_JPY", param
            )  # 時間昇順(直近が最後尾）
            if d60_df_res["error"] == -1:
                print("error Candle")
            ...
            d30_df_res = self.base_oa.InstrumentsCandles_exe(
                "USD_JPY", param
            )  # 時間昇順(直近が最後尾）
            if d30_df_res["error"] == -1:
                print("error Candle")
...
class eachCandleAnalysis:
    ...
    def current_place(self):
        ...
        """
        self.peaks_class.peak_strength_sort()
        # print("    current price")
        # print("マイナス方向")
    ...
    def cal_move_size(self):
        ...
        if len(self.peaks_class.peaks_original) == 1:
            ...
            target_peak = self.peaks_class.peaks_original[
                1
            ]  # ビッグムーブ検査の対象となるのはひとつ前のピーク
        if self.peaks_class.peaks_original[0]["count"] == 2:
            # 重複オーダーとなる可能性をここで防止するため、ビッグムーブの判定はLatestカウントが2の場合のみ
            if (
                target_peak["gap"] >= self.fluctuation_gap
                and target_peak["count"] <= self.fluctuation_count
            ):
        ...
        sum(item["gap"] for item in filtered_peaks) / len(filtered_peaks)
        # 最大値と最小値
        max_index, max_peak = max(
            enumerate(filtered_peaks[:]), key=lambda x: x[1]["peak"]
        )
        min_index, min_peak = min(
            enumerate(filtered_peaks[:]), key=lambda x: x[1]["peak"]
        )
        # 最大変動と最小変動
        max_gap_index, max_gap = max(
            enumerate(filtered_peaks[:]), key=lambda x: x[1]["gap"]
        )
        min_gap_index, min_gap = min(
            enumerate(filtered_peaks[:]), key=lambda x: x[1]["gap"]
        )
        [item for item in filtered_peaks[:] if item != max_gap]
        # print(t6, "検出範囲ピーク",)
