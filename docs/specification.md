...
時計はJST。ループ間隔は1秒です。
ペア別スプレッド上限は`trading.spread_limit_pips`で指定し、
未指定ならUSD_JPY=1.1、EUR_USD=1.5、AUD_USD=1.8 pipsを使用します。
original・Matcha・プラグイン共通で、バックテストも明示したconfigから同じ値を使用します。
設定は起動時に確定し、価格桁での丸めと上限同値の許可は従来どおりです。
