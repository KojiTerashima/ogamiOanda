...
class order_information:
    ...
    latest_df_r = None
    latest_df_get_time = datetime.datetime.now().replace(microsecond=0) - timedelta(
        minutes=1
    )
    ...
    history_plus_minus = [
        0
    ]  # 空だと、過去のプラスマイナスを参照するときおかしなことになるので０を入れておく
    ...
    def __init__(self, name, is_live, oanda_factory=None, notifier: Notifier | None = None):
        ...
        self.u = 3
        self.lc_ensure_range_border = (
            0.01  # lc_changeで確保できる利益が、この値以下の場合は、実施しない。
        )
        # self.reset()
        ...
        self.life = (
            False  # 有効かどうか（オーダー発行からポジションクローズまでがTrue）
        )
        ...
        self.t_state = ""
        self.t_type = (
            ""  # 結合ポジションか？　plantとinitialとcurrentのユニットの推移でわかる？
        )
        ...
        self.win_lose_border_range = (
            0  # この値を超えている時間をWin、以下の場合Loseとする
        )
        ...
        self.lc_change_candle_foot = (
            "M5"  # Candleロスカット底上げを行う場合、どのキャンドルを使うかを指定。
        )
        self.lc_change_candle_exe_at_minus = (
            False  # マイナス域で、キャンドルLC＿Changeを実行する
        )
        ...
        self.watching_out_time_border = (
            60  # startから、この時間経過でwatch終了＆Order終了（order終了優先）
        )
        ...
        self.send_line_exe = (
            True  # いるか不明（Trueの場合は一部の情報についてLINE送信頻度増）
        )
        self.memo = ""
    ...
    def reset(self):
        ...
        self.lc_change_from_candle_lc_price = 0
        self.lc_change_num = (
            0  # LCChangeまたはLCChangeCandleのいずれかの執行でTrueに変更される
        )
        self.lc_change_less_minus_done = False
    ...
    def order_plan_registration(self, order_class):
        ...
        self.name = plan[
            "name"
        ]  #  + "(" + str(self.priority) + ")"  # 名前を入れる(クラス内の変更）
        ...
        if hasattr(self, "candle_analysis_class"):
            ...
            self.move_ave5 = round(
                self.candle_analysis_class.candle_class.cal_move_ave(1), self.u
            )
            self.move_ave60 = round(
                self.candle_analysis_class.candle_class_hour.cal_move_ave(1), self.u
            )
    ...
    def make_order(self):
        ...
        if order_ans["cancel"]:  # キャンセルされている場合は、リセットする
            self.send_line(
                " 　Order不成立（今後ループの可能性）", self.name, order_ans["order_id"]
            )
            self.o_id = 0
        ...
        return {
            "order_name": "",
            "order_id": order_ans["order_id"],
            "order_result": order_ans,
        }
    ...
    def make_order_info(self):
        ...
        if self.o_id == 0:
            ...
            self.for_line_send = (
                ...
                + "】を即時ポジションなしで発行"
                + "指定価格:【"
                + str(round(self.plan_json["target_price"], self.u))
                + "】"
                + ",DIR:"
                + str(self.plan_json["direction"])
                + ", 数量:"
                + str(self.plan_json["units"])
                + ", TP:"
                + str(round(self.plan_json["tp_price"], self.u))
                + "("
                + str(round(self.plan_json["tp_range"], self.u))
                + ")"
                + ", LC:"
                + str(round(self.plan_json["lc_price"], self.u))
                + "("
                + str(round(self.plan_json["lc_range"], self.u))
                + ")"
                + ", AveMove:"
                + str(round(self.plan_json["move_ave"], self.u))
                + ", memo:"
                + str(self.memo)
                + ",lc_change"
                + str(lc_change_str)
            ...
            oanda_res = self.order_result_from_oanda[
                "orderCreateTransaction"
            ]  # =self.order_result_wrap_up['json']
            # tempの計算
            tp_range = round(
                abs(
                    float(oanda_res["takeProfitOnFill"]["price"])
            ...
            lc_range = round(
                abs(
                    float(oanda_res["stopLossOnFill"]["price"])
                    - float(order_result["price"])
                ),
                self.u,
            )
            self.for_line_send = (
                ...
                + "】,\n"
                + "指定価格:【"
                + str(self.plan_json["target_price"])
                + "】"
                + ", 数量:"
                + str(oanda_res["units"])
                + ", タイプ:"
                + str(self.plan_json["type"])
                + ", TP:"
                + str(oanda_res["takeProfitOnFill"]["price"])
                + "("
                + str(tp_range)
                + ")"
                + ", LC:"
                + str(oanda_res["stopLossOnFill"]["price"])
                + "("
                + str(lc_range)
                + ")"
                + ", AveMove:"
                ...
                + str(self.o_id)
                + ", 取得価格:"
                + str(order_result["execution_price"])
                + ",\n"
                + ", memo:"
                + str(self.memo)
                + "lc_change"
    ...
    def close_order(self):
        ...
        self.life_set(
            False
        )  # まずはクローズ状態にする　（エラー時の反復を防ぐため。ただし毎回保存される？？）
    ...
    def after_close_trade_function(self):
        ...
        if trade_latest["state"] == "CLOSED":
            # Totalの円を求める
            order_information.total_yen = round(
                order_information.total_yen + float(trade_latest["realizedPL"]), 2
            )
            if order_information.total_yen > order_information.total_yen_max:
                order_information.total_yen_max = (
                    order_information.total_yen
                )  # 過去の勝ちの最大値を取得しておく(気分的に）
            ...
            # TotalのPLuを求める
            order_information.total_PLu = round(
                order_information.total_PLu + trade_latest["PLu"], self.u
            )
            ...
            if float(trade_latest["realizedPL"]) < 0:
                order_information.minus_yen_position_num = (
                    order_information.minus_yen_position_num + 1
                )
            else:
                order_information.plus_yen_position_num = (
                    order_information.plus_yen_position_num + 1
                )
        else:
            # 価格情報の更新
            order_information.total_yen = round(
                order_information.total_yen + float(trade_latest["unrealizedPL"]), 2
            )
            order_information.total_PLu = round(
                order_information.total_PLu + trade_latest["PLu"], self.u
            )
        ...
        order_information.history_name_plus_minus.append(
            {"name": self.name[:-5], "PLu": trade_latest["PLu"]}
        )
        ...
        units_for_view = abs(float(trade_latest["initialUnits"])) - abs(
            float(trade_latest["currentUnits"])
        )
        # print("　　　　classPosition 540行目", float(trade_latest['initialUnits']), trade_latest['currentUnits'], self.o_json)
        direction = float(trade_latest["initialUnits"]) / abs(
            float(trade_latest["initialUnits"])
        )
        ...
        if trade_latest["state"] == "CLOSED":
            ...
            res2 = (
                "【決:"
                + str(trade_latest["averageClosePrice"])
                + ", "
                + "取:"
                + str(trade_latest["price"])
                + "】"
            )
            res3 = (
                "【ポジション期間の最大/小の振れ幅】 ＋域:"
                + str(self.win_max_plu)
                + "/ー域:"
                + str(self.lose_max_plu)
            )
            res3 = (
                res3
                + " 保持時間(秒)"
                + str(trade_latest["time_past"])
                + ",LC change:"
                + self.lc_change_str
            )
            res4 = (
                "【今回結果】"
                + str(trade_latest["PLu"])
                + ","
                + str(trade_latest["realizedPL"])
                + "円\n"
            )
            res5 = (
                "【合計】計"
                + str(order_information.total_PLu)
                + ",計"
                + str(order_information.total_yen)
                + "円"
            )
            res6 = (
                "【合計】累積最大円:"
                + str(order_information.total_yen_max)
                + ",最小円:"
                + str(order_information.total_yen_min)
            )
            res7 = (
                "【合計】累計最大PL:"
                + str(order_information.total_PLu_max)
                + ",最小PL:"
                + str(order_information.total_PLu_min)
            )
            res8 = (
                "【回数】＋:"
                + str(order_information.plus_yen_position_num)
                + ",―:"
                + str(order_information.minus_yen_position_num)
            )
            self.send_line(
                "■■■解消:",
                self.name,
            ...
            res1 = "【Unit】" + str(
                trade_latest["currentUnits"]
            )  # )str(units_for_view * direction)
            id_info = "【orderID】" + str(self.o_id) + "【tradeID】" + str(self.t_id)
            res2 = (
                "【決:"
    ...
    def close_trade(self, units=None):
        ...
        # 部分解除の場合（LINE送信無し）
            ...
            if float(units) > abs(float(self.t_current_units)):
                ...
                self.send_line(
                    "    指定されたCloseUnits大（Error)⇒全解除, units",
                    ">",
                    abs(float(self.t_current_units)),
                )
                self.life_set(
                    False
                )  # ★大事　全解除の場合、APIがエラーでもLIFEフラグは取り下げる
                self.t_state = "CLOSED"
                close_res = self.oa.TradeClose_exe(self.t_id, None)  # ★オーダーを実行
                return 0
            else:
                # 部分解消が正常に実行できる注文数の場合
                close_res = self.oa.TradeClose_exe(
                    self.t_id, {"units": units}
                )  # ★オーダーを実行
                self.send_line("部分解消")
            if close_res["error"] == -1:  # APIエラーの場合終了。ただし、
                self.send_line(
                    "  ★ポジション解消ミス部分＠close_position",
                    self.name,
                    self.t_id,
                    self.t_pl_u,
                )
            ...
            res_json["orderFillTransaction"]["tradeReduced"]["tradeID"]
    ...
    def detect_change(self):
        ...
        if (self.o_state == "PENDING" or self.o_state == "") and order_latest[
            "state"
        ] == "FILLED":  # オーダー達成（Pending⇒Filled）
            if trade_latest["state"] == "OPEN":  # ポジション所持状態
                self.send_line("    (取得)", self.name, trade_latest["price"])
                #  取得時には、ローソクデータも取得しておく
                now = datetime.datetime.now().replace(microsecond=0)
                time_difference = (
                    now - self.latest_df_get_time
                )  # 最後にDFを取った時からの経過時間
                seconds_difference = abs(time_difference.total_seconds())
                if seconds_difference >= 30:
                    # 初回の場合はやる
                    self.is_first_time_lc_change_candle = (
                        False  # 二回目以降はFalseにFalseを入れることになるが、、
                    )

                    d5_df = self.oa.InstrumentsCandles_multi_exe(
                        "USD_JPY",
                        {"granularity": "M5", "count": self.lc_change_candle_num},
                        1,
                    )  # 時間昇順(直近が最後尾）
                    if d5_df["error"] == -1:
                        print("error Candle")
                        ...
                        if self.is_first_time_lc_change_candle:
                            ...
                            self.latest_df_get_time = datetime.datetime.now().replace(
                                microsecond=0
                            )
            ...
            if "position_state" in trade_latest:
                if (
                    trade_latest["position_state"] == "CLOSED"
                ):  # ポジションクローズ（ポジション取得とほぼ同時にクローズ[異常]）
            ...
            self.t_state == "OPEN" and trade_latest["state"] == "CLOSED"
        ):  # 通常の成り行きのクローズ時
            print("    成り行きのクローズ発生")
            self.after_close_trade_function()
            return 0
    ...
    def order_update_and_close(self):
        ...
        if order_latest["state"] == "PENDING":
            ...
            if self.o_time_past_sec > self.order_timeout_min * 60 and (
                self.o_state == "" or self.o_state == "PENDING"
            ):
                self.close_order()
                self.send_line(
                    "   オーダー解消(時間)@",
                    self.name,
                    self.o_time_past_sec,
                    ",",
                    self.order_timeout_min,
                    position_check_no_args()["name_list"],
                )
                result_dic = {
                    "name": self.name,
    ...
    def update_information(
        ...
        if self.o_id == -1:
            ...
            if "error" in order_ans:
                if order_ans["error"] == 1:
                    ...
                    # エラー対応
                    if (
                        self.update_information_error_o_id_num >= 3
                        ...
                        print(
                            "OrderErrorのためリターン０（@classPosition898）ID:",
                            self.o_id,
                            type(self.update_information_error_o_id),
                        )
                        self.update_information_error_o_id_num = (
                            self.update_information_error_o_id_num + 1
                        )
                        if self.o_id == 0:
                            print("o_idがIでおかしい")
                        else:
            ...
            if order_latest["state"] == "FILLED":
                ...
                if "tradeOpenedID" in order_latest:
                    # ポジションが存在している場合
                    # 他のオーダーを完全にクローズさせた場合
                    if "tradeClosedIDs" in order_latest and not (
                        self.already_offset_notice
                    ):
                    ...
                    if trade_ans["error"] == 1:
                        print(
                            "    トレード情報取得Error＠update_information", self.t_id
                        )
                        return 0
                    trade_latest = trade_ans["data"]["trade"]  # Jsonデータの取得
                    self.t_json = trade_latest  # Json自体も格納
                    ...
                    trade_latest = self.oa.TradeDetails_exe(reduced_trade_id)["data"][
                        "trade"
                    ]  # ■■API
                    # print("トランザクション")
                    ...
                    trade_latest["state"] = "CLOSE"
                    trade_latest["PLu"] = round(
                        float(trade_latest["unrealizedPL"])
                        / abs(
                            float(transaction_info["transactions"][0]["requestedUnits"])
                        ),
                        2,
                    )
                    self.t_json = trade_latest  # Json自体も格納
        ...
        if self.o_state == "FILLED" and self.t_state == "CLOSED" and self.life:
            self.life_set(True)
            self.send_line(
                "Filled Closed Trueの謎状態あり⇒強制的にLifeにFalseを入れて終了　classPosition 537行目"
            )
        # 変化による情報（勝ち負けの各最大値、継続時間等の取得）
        self.updateWinLoseTime(
            trade_latest["PLu"]
        )  # PLU(realizePL / Unit)の推移を記録する
    ...
    def update_information_at_out_time(
        self, candle_analysis_class=None
    ):  # orderとpositionを両方更新する
        """
    ...
    def watching_for_position_make_order(self):
        ...
        if "order_result" in order_res:
            ...
            line_send = (
                ...
                + str(o_trans["units"])
                + ", TP:"
                + str(o_trans["takeProfitOnFill"]["price"])
                + "("
                + str(
                    round(
                        abs(
                            float(o_trans["takeProfitOnFill"]["price"])
                            - float(self.plan_json["target_price"])
                        ),
                        self.u,
                    )
                )
                + ")"
                + ", LC:"
                + str(o_trans["stopLossOnFill"]["price"])
                + "("
                + str(
                    round(
                        abs(
                            float(o_trans["stopLossOnFill"]["price"])
                            - float(self.plan_json["target_price"])
                        ),
                        self.u,
                    )
                ...
                + ")"
                + ", OrderID:"
                + str(order_res["order_id"])
                + ", Alert:"
                + str(self.alert_range)
                + "("
                + str(round(self.alert_price, self.u))
                + ")"
                + ", 取得価格:"
                + str(order_res["order_result"]["execution_price"])
                + ",\n"
            )
            self._notifier.notify(line_send)
        else:
            order_res["order_result"] = "この処理はオーダー失敗の可能性大"
    ...
    def watching_for_position(self, candle_analysis_class):
        ...
        if (
            self.o_state != "Watching" or self.watching_for_position_done
        ):  # 足数×〇分足×秒
        ...
        if self.plan_json["type"] == "STOP":
            if (
                round(self.step1_keeping_second, 0) % 15 == 0
                and round(self.step1_keeping_second, 0) != 0
            ) or 1 <= round(self.step1_keeping_second, 0) <= 2:
                pass
            ...
            if gap_seconds_from_order_regist > self.order_timeout_min * 60:
                ...
                if (
                    gap_seconds_from_start_watching > self.watching_out_time_border
                    or gap_seconds_from_start_watching == 0
                ):
                    # ウォッチの時間も経過している場合
                    ...
                    self.close_order()
                    self._notifier.notify("ウォンチングのみのオーダーを解消", self.name)
                    return 0
                else:
            ...
            if not self.watching_position_done_send_line:
                self._notifier.notify(
                    "初回のポジションウォッチング状態(STOP):",
                    self.name,
                    self.plan_json["direction"],
                    self.plan_json["target_price"],
                    self.current_price,
                )
            ...
            if self.step2_filled:
                ...
                if self.step1_filled:
                    ...
                    if self.step1_keeping_second >= 10:
                        ...
                        if (o_dir == 1 and now_price > temp_price) or (
                            o_dir == -1 and now_price < temp_price
                        ):
                        ...
                        print(
                            "  STEP1 時間経過待ち", now_time, self.step1_keeping_second
                        )
                        if (o_dir == 1 and now_price > temp_price) or (
                            o_dir == -1 and now_price < temp_price
                        ):
    ...
    def change_tp(self, tp_margin):
        ...
        """
        tp_price = round(
            float(self.t_execution_price) + (tp_margin * self.plan_json["direction"]),
            self.u,
        )
        data = {
        ...
        self.send_line(
            ...
            self.name,
            self.t_pl_u,
            round(self.plan_json["tp_price"], self.u),
            "⇒",
            tp_price,
            "tp_range:",
            round(tp_margin, self.u),
        )
    ...
    def lc_change(
        self,
    ):  # ポジションのLC底上げを実施 (基本的にはUpdateで平行してする形が多いかと）
        """
        ...
        if (
            self.lc_change_candle_done
        ):  # 既にキャンドルLCChangeに行った場合は、執行しない
            return 0
        ...
        for i, item in enumerate(self.lc_change_dic_arr):
            ...
            if not lc_exe:  # or self.t_time_past_sec > lc_change_till_sec:
                ...
                status_res = status_res + gene.str_merge(
                    "[",
                    i,
                    "] 指定なし",
                    "lc_exe:",
                    lc_exe,
                    lc_change_waiting_time_sec,
                    ",",
                    "現pl" + str(self.t_pl_u),
                    ",指定Trigger",
                    lc_trigger_range,
                ...
                datetime.datetime.now() - item["time_done"]
                continue
            elif (
                self.t_time_past_sec < lc_change_waiting_time_sec
                or self.t_time_past_sec > lc_change_till_sec
            ):
            ...
            if self.t_pl_u >= lc_trigger_range:
                ...
                )  # このクラス自体にLCChangeを実行した後をつけておく（カウント）
                # これで配列の中の辞書って変更できるっけ？？
                item["done"] = True
                item["time_done"] = datetime.datetime.now()
                new_lc_price = round(
                    float(self.t_execution_price)
                    + (lc_ensure_range * self.plan_json["direction"]),
                    self.u,
                )
                data = {
                    "stopLoss": {"price": str(new_lc_price), "timeInForce": "GTC"},
                }
                res = self.oa.TradeCRCDO_exe(self.t_id, data)  # LCライン変更の実行
                if res["error"] == -1:
                    self.send_line("    LC変更ミス＠lc_change")
                ...
                if self.send_line_exe:
                    self.send_line(
                        ...
                        self.name,
                        self.t_pl_u,
                        round(self.plan_json["lc_price"], self.u),
                        "⇒",
                        new_lc_price,
                        "Border:",
                        round(lc_trigger_range, self.u),
                        "保証",
                        round(lc_ensure_range, self.u),
                        "Posiprice",
                        self.t_execution_price,
                        "予定価格",
                        round(self.plan_json["target_price"], self.u),
                    )
                # LC Priceの入れ替え
    ...
    def lc_change_from_candle(
        self, candle_analysis_class
    ):  # ポジションのLC底上げを実施 (基本的にはUpdateで平行してする形が多いかと）
        """
        ロスカット底上げを実施する。セルフとレールに近い
        ...
        if (
            self.t_state != "OPEN" or self.t_time_past_sec < exe_after_min
        ):  # 足数×〇分足×秒
        ...
        if (
            peaks[0]["count"] >= 3
            and peaks[0]["direction"] == self.plan_json["direction"]
        ):
            # df_r.iloc[1]['low']の-2が選択できる状態であれば、実行する
            if self.plan_json["direction"] > 0:
                # 買い方向の場合、ひとつ前のローソクのLowの値をLC価格に
                lc_price_temp = (
                    float(df_r.iloc[soeji]["low"]) - order_information.add_margin
                )
            else:
                # 売り方向の場合、ひとつ前のローソクのHighの値をLC価格に
                lc_price_temp = (
                    float(df_r.iloc[soeji]["high"]) + order_information.add_margin
                )
        ...
        lc_range = round(
            abs(float(lc_price_temp) - float(self.t_execution_price)), self.u
        )
        self.lc_change_num = (
            self.lc_change_num + 1
        )  # このクラス自体にLCChangeを実行した後をつけておく（カウント）
        if self.send_line_exe:
            self.send_line(
                ...
                self.plan_json["target_price"],  # )
                "参考にされた時間",
                df_r.iloc[soeji]["time_jp"],
                "添え字",
                soeji,
                "オープン:",
                df_r.iloc[soeji]["open"],
                "クローズ:",
                df_r.iloc[soeji]["close"],
            )
        # LC Priceの入れ替え
        self.plan_json["lc_price"] = new_lc_price
    ...
    def catch_exist_position(self, name, oa_mode, priority, json):
        ...
        self.plan_json["direction"] = int(json["currentUnits"]) / abs(
            int(json["currentUnits"])
        )
        ...
        self.name = (
            ...
            + "_"
            + str(
                gene.delYearDay(datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
            )
        )
    ...
    def linkage_tp_change(self, new_tp_price):
        ...
        data = {
            "takeProfit": {"price": str(new_tp_price), "timeInForce": "GTC"},
        }
        res = self.oa.TradeCRCDO_exe(self.t_id, data)  # LCライン変更の実行
        if res["error"] == -1:
    ...
    def linkage_forced_lc_change_setting(self, main_plu, pl_u):
        ...
        self.lc_change_from_candle_lc_price = (
            0  # キャンドル変更済の場合０以外。０にすることで、LcChangeに戻す
        )
        self.lc_change_candle_done = (
            False  # キャンドル変更がTrueの場合通常lc_Changeは実行されないため、戻す
        )
        # 執行内容の準備
        if main_plu < 0:
        ...
        tr = (
            main_plu * -1
        )  # 想定されているのはマイナス域で、どれだけマイナスを減らすか。

        self.lc_change_dic_arr = [
            {
                "exe": True,
                "time_after": 0,
                "trigger": round(tr / 2, self.u),
                "ensure": round(tr, self.u),
            },  # 負けの半分まで行ったら、、
            {
                "exe": True,
                "time_after": 0,
                "trigger": round(tr / 3, self.u),
                "ensure": round((tr / 3) * 2, self.u),
            },
        ]
        if self.send_line_exe:
            self.send_line(
                "　(Linkage　マイナス域用LcChangeへ)",
                self.name,
                "トリガ",
                round(tr / 2, self.u),
                "確保",
