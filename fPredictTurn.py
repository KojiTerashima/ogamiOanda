...
def cal_predict_turn_at_trend(peaks_class):
    ...
    first_margin = 0.01
    search_grid = 0.02
    search_range_upper = round(
        search_grid / 2, 3
    )  # 0.01  # searchPriceの上下この範囲内でのピークを探す
    ...
    if r["count"] == 2:
        ...
        while loop_manegement <= search_range:  # ★ループ処理
            ...
            # 各調査での結果を集計＆格納
            each_range_peaks = sorted(
                each_range_peaks,
                key=lambda x: x["latest_body_peak_price"],
                reverse=(r["direction"] * -1 == 1),
            )
            suggested_peaks.append(
                {"base_price": round(search_price, 3), "peaks": each_range_peaks}
            )
    ...
    for each in suggested_peaks:
        ...
        if (
            strength > max_peak_streng
        ):  # >=にすると遠いほうで上書きされるので、一番近くで、Strengthが強いところを求める
    ...
    print(
        far_price,
        "差分",
        round(abs(far_price - max_base_price), 3),
        "強度",
        far_peak_strength,
    )
    if max_base_price == 0:
    ...
    order["order_timeout_min"] = (
        45  # 45分でオーダー消去（すぐに越えてない場合はもうNGとみなす。最大10分か？）
    )
