...
class main:
    def __init__(self):
        ...
        self.latest_exe_time = datetime.datetime.now().replace(
            microsecond=0
        )  # 最終実行時刻を取得しておく
        ...
        self.start_time_str = (
            ...
            + "_"
            + str(start_time.hour).zfill(2)
            + str(start_time.minute).zfill(2)
        )
        # 価格系
    ...
    def get_time_info(self):
        ...
        self.now_str = (
            ...
            + "_"
            + str(self.now.hour).zfill(2)
            + str(self.now.minute).zfill(2)
            + "_"
            + str(self.now.second).zfill(2)
        )
    ...
    def force_order(self):
        ...
        # ■オーダーの発行
        print("test")
        exe_res = self.positions_control_class.order_class_add(
        ...
        if exe_res == 0:
            ...
            self._notifier.notify(
                ...
                str(self.now_spread),
                "直前の結果:",
                classPosition.order_information.before_latest_plu,
                ",開始時間",
                self.start_time_str,
            )
    ...
    def mode1(self):
        ...
        """
        print(
            "■■■■■■5分ごと調査■■■■", self.now, self.past_time_from_latest_mode1_exe
        )  # 表示用（実行時）
        ...
        analysis_result_instance = am.wrap_all_analysis(
            self.candleAnalysisClass, self.positions_control_class, "live"
        )
        # ■ オーダー発行
    ...
    def exe_manage(self):
        ...
        if not self.first_exe:
            ...
            self.past_time_from_latest_mode1_exe = (
                datetime.datetime.now().replace(microsecond=0) - self.latest_exe_time
            ).seconds
            ...
            if is_only_update_mode:
                ...
                if (
                    ...
                    and self.past_time_from_latest_mode1_exe > 60
                ):
                    print("  ")
                    print("  ")
                    self.mode1()  # ★★Mode1の実行
                ...
                if (
                    self.time_min % 1 == 0 and self.time_sec % 2 == 0
