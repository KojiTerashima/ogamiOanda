...
class MainAnalysis:
    ...
    def predict_registance_turn_analysis(self):
        ...
        if (
            peaks[0]["direction"] * -1 == 1
        ):  # ポジション予定の方向が買いの場合、最低価格をLC価格候補にする
        ...
        if peaks[0]["direction"] == 1:
            ...
            target_price = round(
                target_line_price + m, self.round_digit
            )  # +の値で、取得しにくい方向へ
        else:
    ...
    def simple_turn_analysis(self):
        ...
        # 数字に調整が入る場合
        if (
            df.iloc[1]["moves"] >= big_move_criteria
            or df.iloc[2]["moves"] >= big_move_criteria
        ):
        ...
        if latest_dir == 1:
            target_bb_price = round(
                df.iloc[1]["bb_upper"], self.round_digit
            )  # [0]のほうが良い？
        else:
            target_bb_price = round(df.iloc[1]["bb_lower"], self.round_digit)
        gap_bb_to = round(
            abs(latest_price - target_bb_price), self.round_digit
        )  # 狭ければNG？
        ...
        for i in range(2, 4):  # 最大2回（4-2)、ただしturnのカウント以下
            print(
                s, "  dfの時間：", df.iloc[i]["time_jp"], i, count, df.iloc[i]["body"]
            )
        ...
        if peaks[0]["gap"] >= 0.05:
            if rt_ratio_new >= 1.1:
                print(
                    s,
                    "latestが大きく、rtの比率も大きく、戻りが強いと判断？ gap",
                    peaks[0]["gap"],
                    "rt",
                    rt_ratio_new,
                )
        ...
        if not r_flag:
            ...
            if s_dir == 1:
                s_target_price = latest[
                    "latest_wick_peak_price"
                ]  # latest['highest']と比較してもいいかも？？
                ...
                s_target_price = s_target_price - real_margin
            target_margin = round(
                abs(s_target_price - self.latest_price), self.round_digit
            )
            s_lc_range = round(
                abs(s_lc_price - s_target_price), self.round_digit
            )  # 計算用
            s_for_lc_change = {
                "exe": True,
                "time_after": 0,
                "trigger": s_lc_range + 0.01,
                "ensure": s_lc_range * 0.5,
            }
            if s_lc_range >= 0.09:  # lcが大きすぎる場合は、補正する(機能の準備のみ）
                ...
                if s_dir == 1:
                    print(
                        ...
                        "LCrangeが広すぎるので修正",
                        s_dir,
                        s_target_price - arrowed_lc_range,
                    )
                else:
                    print(
                        s,
            ...
            lc_range_r = (
                target_margin_for_r * 3
            )  # 初期予定するLCレンジ（ただし、最低でもturnのhighかを知りたい ）
            if r_dir == 1:
                # r_target_price = river['highest']  # ここは突破しにくいラインで、うまくいかなかった
                r_target_price = (
                    self.latest_price + target_margin_for_r
                )  # ＋正の値で取得しにくい方向へ
                lc_range_r_hl = abs(
                    r_target_price - peaks[1]["highest"]
                )  # 最低でもこの分のLCrangeは確保したい
            ...
            print(
                s,
                "  ５分足トレンドについて is_previous",
                bb_m5_class.is_previous,
                "向き",
            ...
            print(
                s,
                "  分足のトレンドがあり、latestの向きが異なるため、逆張り",
                bb_m5_class.latest_price_position_in_bb,
            )
            ...
            order_class = OCreate.Order(
                {
                    ...
                    "lc": round(
                        abs(s_target_price - r_target_price) * 1.1, self.round_digit
                    ),  # _rと重複するように,
                    "lc_change": self.make_lc_change_dic(
            ...
            order_class_r = OCreate.Order(
                {
                    ...
                    "lc": target_margin_for_r
                    * 2.5,  # 0.03,  # ave5.cal_move_ave(1),  # width,
            ...
            if r_dir == 1:
                # r_target_price = river['highest']  # ここは突破しにくいラインで、うまくいかなかった
                r_target_price = (
                    self.latest_price + 0.02
                )  # target_margin_for_r  # ＋正の値で取得しにくい方向へ
