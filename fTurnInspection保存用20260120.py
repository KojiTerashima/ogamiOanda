...
class MainAnalysis:
    ...
    def predict_registance_turn_analysis(self):
        ...
        print(
            s,
            "バスタブ型確認 (２点、最寄りの１つが30分以内、もう一つが５時間以上前、かつ　全部で３個以内",
        )
    ...
    def simple_turn_analysis(self):
        ...
        turn = peaks[2]
        turn_df = df[
            latest["count"] + river["count"] - 1 : latest["count"]
            + river["count"]
            + turn["count"]
            - 1
        ]
        flop = peaks[3]
        flop_df = df[
            latest["count"] + river["count"] + turn["count"] - 2 : latest["count"]
            + river["count"]
            + turn["count"]
            + flop["count"]
            - 2
        ...
        print(
            s,
            "latest",
            latest["latest_time_jp"],
            "river",
            river["latest_time_jp"],
            "turn",
            turn["latest_time_jp"],
        )
        # BBとクロスしているかを確認する
        print(
            s, "latest_df", latest_df.iloc[0]["time_jp"], latest_df.iloc[-1]["time_jp"]
        )
        ...
        if river_cross_count >= river_cross_border and river["count"] >= 3:
            ...
            older = peaks[3]
            lm_gap_ratio = round(
                middle["gap"] / later["gap_high_low"], self.round_digit
            )
            om_gap_ratio = round(
                middle["gap"] / older["gap_high_low"], self.round_digit
            )
            if lm_gap_ratio <= 0.4 and om_gap_ratio <= 0.4:
                print(
                    "対象のmiddle(2)は微折り返し認定⇒今回で二回目のため、レンジ警戒",
                    lm_gap_ratio,
                    om_gap_ratio,
                )
                cross_range = True
        ...
        if not r_flag:
            ...
            r_dir = peaks[0][
                "direction"
            ]  # 折り返したままlatestのほうに行く(riverで抵抗を受ける側）
            target_margin = round(
                self.ca5.cal_move_ave(1) * 0.2, self.round_digit
            )  # 0.02
            if r_dir == 1:
                ...
                r_lc_range = round(
                    max(r_lc_range1, r_lc_range2) + 0.004, self.round_digit
                )  #  + 0.02
                lc_price_temp = r_target_price - r_lc_range
                print(s, "target_base", df1["high"], "margin", target_margin)
                print(
                    s, "latest low", latest["lowest"], "river_lowest", river["lowest"]
                )
            else:
                ...
                print(
                    s, "latest high", latest["highest"], "river_high", river["highest"]
                )
            wick_range = abs(
                river["latest_wick_peak_price"] - river["latest_body_peak_price"]
            )
            print(
                s, "wickRange", wick_range
            )  # 長すぎる場合は、ちょっとLCを短くする検討？？
            # lc_changeの設定
            print(
                s,
                "lcChange確認用",
            ...
            s_for_lc_change = [
                # {"exe": True, "time_after": 605, "trigger": -0.05, "ensure": -r_lc_range},  # 5pipまでは許容したい。
                {
                    ...
                    "time_after": 605,
                    "trigger": temp,
                    "ensure": round(temp * 0.8, self.round_digit),
                },  # これは要検討
                {
                    "exe": True,
                    "time_after": 605,
                    "trigger": r_lc_range,
                    "ensure": r_lc_range * 0.8,
                },
            ]
    ...
    def support_line_detect(self, target_no):
        ...
        sum(d["item"]["peak_strength"] for d in same_price_list)
