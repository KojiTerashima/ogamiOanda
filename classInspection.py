...
class Inspection:
    def __init__(
        ...
        self.oa = classOanda.Oanda(
            tk.accountIDl, tk.access_tokenl, "live"
        )  # クラスの定義
        ...
        self.analysis_num_m5 = (
            180  # この足の分のデータフレームを処理する（足ごとに設定）
        )
        self.analysis_num_h1 = (
            240  # この足の分のデータフレームを処理する（足ごとに設定）
        )
        ...
        self.gl_start_time_str = (
            str(self.gl_now.month).zfill(2)
            + str(self.gl_now.day).zfill(2)
            + "_"
            + str(self.gl_now.hour).zfill(2)
            + str(self.gl_now.minute).zfill(2)
            + str(self.gl_now.second).zfill(2)
        )

        self.filepath = (
            tk.folder_path + self.gl_start_time_str + "_main_analysis_temp.csv"
        )
        self.positions_control_class = pc.position_control_for_test(
            True, self.filepath
        )  # ポジションリストの用意
    ...
    def get_data(self):
        ...
        if self.gl_exist_data:
            # 既存の5分足データを取得
            self.gl_d5_df = pd.read_csv(
                self.gl_main_csv_path, sep=",", encoding="utf-8"
            )
            self.gl_5m_start_time = self.gl_d5_df.iloc[0]["time_jp"]
            self.gl_5m_end_time = self.gl_d5_df.iloc[-1]["time_jp"]
            self.gl_actual_5m_start_time = self.gl_d5_df.iloc[self.gl_need_to_analysis][
                "time_jp"
            ]
            self.gl_d5_df_r = self.gl_d5_df.sort_index(
                ascending=False
            )  # 時系列を逆にしたものが解析用！
            ...
            print(
                "実際の解析時間は",
                self.gl_d5_df.iloc[self.gl_need_to_analysis]["time_jp"],
                "-",
                self.gl_5m_end_time,
            )
            # 既存の5秒足データを取得
            ...
            self.gl_h1_df_r = self.gl_h1_df.sort_index(
                ascending=False
            )  # 時系列を逆にしたものが解析用！
            ...
            euro_time_datetime_iso = (
                str(euro_time_datetime.isoformat()) + ".000000000Z"
            )  # ISOで文字型。.0z付き）
            params = {
                "granularity": "M5",
                "count": self.gl_m5_count,
                "to": euro_time_datetime_iso,
            }  # コツ　1回のみ実行したい場合は88
            data_response = self.oa.InstrumentsCandles_multi_exe(
                "USD_JPY", params, self.gl_m5_loop
            )
            ...
            self.gl_d5_df.to_csv(
                tk.folder_path
                + gene.str_to_filename(self.gl_5m_start_time)
                + "_test_m5_df.csv",
                index=False,
                encoding="utf-8",
            )
            ...
            # 5秒足のデータを取得する
            end_time_euro = self.gl_d5_df_r.iloc[0][
                "time"
            ]  # Toに入れるデータ（これは解析用と一致させたいため、基本固定）
            ...
            if all_need_row > 5000:  # 5秒足は、5分足の60倍の行がある
                ...
                s5_count = 5000
                (5000 * loop_for_5s - all_need_row) + (self.gl_need_to_analysis * 60)
                print(
                    "   5S検証：必要な行",
                    all_need_row,
                    "5000行のループ数",
            ...
            data_response = self.oa.InstrumentsCandles_multi_exe(
                "USD_JPY", params, loop_for_5s
            )
            self.gl_s5_df = data_response[
                "data"
            ]  # 期間の全5秒足を取得する  (これは解析に利用しないため、時系列を逆にしなくて大丈夫）
            start_s5_time = self.gl_s5_df.iloc[0]["time_jp"]
            end_s5_time = self.gl_s5_df.iloc[-1]["time_jp"]
            self.gl_s5_df.to_csv(
                tk.folder_path
                + gene.str_to_filename(start_s5_time)
                + "_test_s5_df.csv",
                index=False,
                encoding="utf-8",
            )
            ...
            if all_need_row > 5000:  # 1時間足は、5分足の12分の１の行がある
                ...
                h1_count = 5000
                (5000 * loop_for_1h - all_need_row) + (self.gl_need_to_analysis * 60)
                print(
                    "   1H検証：必要な行",
                    all_need_row,
                    "5000行のループ数",
            ...
            print(params)
            data_response = self.oa.InstrumentsCandles_multi_exe(
                "USD_JPY", params, loop_for_1h
            )
            self.gl_h1_df = data_response[
                "data"
            ]  # 期間の全5秒足を取得する  (これは解析に利用しないため、時系列を逆にしなくて大丈夫）
            self.gl_h1_df_r = data_response["data"].sort_index(
                ascending=False
            )  # 時系列を逆にしたものが解析用！
            start_h1_time = self.gl_h1_df.iloc[0]["time_jp"]
            end_h1_time = self.gl_h1_df.iloc[-1]["time_jp"]
            self.gl_h1_df.to_csv(
                tk.folder_path
                + gene.str_to_filename(start_h1_time)
                + "_test_h1_df.csv",
                index=False,
                encoding="utf-8",
            )
        ...
        d5_df_for_merge = self.gl_d5_df.rename(
            columns=lambda x: f"{x}_y"
        )  # 結合する側の名前にあらかじめ＿ｙをつけておく(予期せぬtime_xができるため）
        self.gl_inspection_base_df = pd.merge(
            self.gl_s5_df,
            d5_df_for_merge,
            left_on="time_jp",
            right_on="time_jp_y",
        ...
        # インデックス以降のデータを取得
        self.gl_inspection_base_df = self.gl_inspection_base_df.loc[
            first_non_nan_index:
        ]
        ...
        for index, row_s5 in self.gl_inspection_base_df.iterrows():
            if index % 1000 == 0:
                print(
                    ...
                    index,
                    "行目/",
                    len(self.gl_inspection_base_df),
                    "中",
                )
            print(
                "■■■検証",
            ...
            if pd.notna(row_s5["time_y"]):  # 毎5の倍数分で解析を実行する
                ...
                # 対応する5分足のデータを取得する
                analysis_df_m5 = find_analysis_dataframe(
                    self.gl_d5_df_r, row_s5["time_jp"]
                )  # 解析用の5分足データを取得する
                ...
                if len(analysis_df_m5) < self.gl_need_to_analysis:
                    ...
                    print(
                        "   結果の解析実施しません",
                        len(analysis_df_m5),
                        "行しかないため（必要行数目安[固定値ではない]",
                        self.gl_need_to_analysis,
                        "データ数",
                        len(analysis_df_m5),
                        "行数:",
                        index,
                ...
                dt = datetime.datetime.strptime(
                    row_s5["time_jp"], "%Y/%m/%d %H:%M:%S"
                )  # 分と秒を0にして、1時間引く
                dt_prev_hour = dt.replace(
                    minute=0, second=0, microsecond=0
                )  # 同じフォーマットで文字列に戻す
                result = dt_prev_hour.strftime("%Y/%m/%d %H:%M:%S")

                analysis_df_h1 = find_analysis_dataframe(
                    self.gl_h1_df_r, result
                )  # 解析用の5分足データを取得する
                ...
                if self.cache_use:
                    ...
                    if current_period is None:
                        ...
                        if os.path.exists(cache_path):
                            ...
                            print(
                                f" {cache_path} を読み込みました",
                                self.cal_passed_minute(temp),
                            )
                            tk.line_send(
                                "[検証期間]読み込み時間",
                                self.cal_passed_minute(temp),
                        ...
                        if save_cache_local:
                            ...
                            if not self.save_cache and save_cache_local:
                                ...
                                tk.line_send(
                                    "[検証期間]このピリオドでまだない物があったと思われる(保存作業発生）",
                                    current_period,
                                    ",",
                                    len_of_others,
                                    "個の新要素 ",
                            ...
                            print(
                                f" {cache_path} を保存完了（{len(self.cache)}件）",
                                self.cal_passed_minute(temp),
                            )
                            tk.line_send(
                                "[検証期間]保存所要時間",
                        ...
                        self.cache = {}
                        gc.collect()  # メモリ解放を強制
                        print("メモリをクリアしました", self.cal_passed_minute(temp))
                        time.sleep(1)  # 2秒間停止
                        ...
                        if os.path.exists(cache_path):
                            ...
                            self.cache = joblib.load(cache_path)
                            print(
                                f" {cache_path} を、月変更タイミングで読み込みました",
                                self.cal_passed_minute(temp),
                            )
                            tk.line_send(
                                "[検証期間]読み込み時間",
                    ...
                    if row_hash in self.cache:
                        ...
                        print(
                            ...
                            len(analysis_df_m5),
                            len(analysis_df_h1),
                            sys.getsizeof(analysis_df_m5),
                            sys.getsizeof(analysis_df_h1),
                        )
                        print("5分足")
                        ...
                        self.candleAnalysisClass = ca.candleAnalisysForTest(
                            "oa", analysis_df_m5, analysis_df_h1
                        )
                ...
                analysis_result_instance = am.wrap_all_analysis(
                    self.candleAnalysisClass, self.positions_control_class
                )
                # ■ オーダー発行
                if not analysis_result_instance.take_position_flag:
                    ...
                    self.positions_control_class.order_class_add(
                        analysis_result_instance.exe_order_classes
                    )
            else:
                # ５の倍数分以外の処理（特になし）
                pass
            ...
            self.positions_control_class.all_update_information(
                row_s5, self.candleAnalysisClass
            )  # positionの更新
    ...
    def cal_result_and_send_line(self):
        ...
        result_df["plus_minus"] = result_df["pl_per_units"].apply(
            lambda x: -1 if x < 0 else 1
        )  # プラスかマイナスかのカウント用
        result_df["order_time_datetime"] = pd.to_datetime(
            result_df["order_time"]
        )  # 文字列の時刻をdatatimeに変換したもの
        ...
        try:
            result_df.to_csv(
                ...
                + self.gl_start_time_str
                + self.memo
                + "_main_analysis_ans.csv",
                index=False,
                encoding="utf-8",
            )
            result_df.to_csv(
                tk.folder_path + "main_analysis_ans_latest.csv",
                index=False,
                encoding="utf-8",
            )
        except Exception:
            print(
                "書き込みエラーあり"
            )  # 今までExcelを開きっぱなしにしていてエラーが発生。日時を入れることで解消しているが、念のための分岐
            result_df.to_csv(
                tk.folder_path + self.gl_start_time_str + "main_analysis_ans.csv",
                index=False,
                encoding="utf-8",
            )
        ...
        # 簡易的な結果表示用
        plus_df = result_df[
            result_df["pl_per_units"] >= 0
        ]  # PLがプラスのもの（TPではない。LCでもトレール的な場合プラスになっているため）
        minus_df = result_df[result_df["pl_per_units"] < 0]
        # 結果表示部
        print(
            "●オーダーの個数", len(result_df)
        )  # , "、約定した個数", len(self.gl_results_list))
        print("●プラスの個数", len(plus_df), ", マイナスの個数", len(minus_df))
        print(
            "●最終的な合計",
            round(result_df["res"].sum(), 3),
            round(result_df["pl_per_units"].sum(), 3),
        )
        print(
            "●実行時間",
            past_time_minute,
        ...
        # LINEを送る
        inspection_day_gap_sec = gene.cal_str_time_gap(
            self.gl_actual_start_time, self.gl_actual_end_time
        )
        ...
        tk.line_send(
            ...
            len(minus_df),
            " 計:",
            len(plus_df) + len(minus_df),
            ",\n",
            "【+域/-域の平均値】",
            round(plus_df["pl_per_units"].mean(), 3),
            ":",
            round(minus_df["pl_per_units"].mean(), 3),
            ",\n",
            "【有意】",
            self.check_skill_difference(len(plus_df), len(minus_df)),
            ",\n",
            "【回数/日】",
            round((len(plus_df) + len(minus_df)) / inspection_day_gap, 0),
            ",\n",
            "【条件】",
            self.memo,
            ",\n参考:処理開始時刻",
        ...
        for _, row in result.iterrows():
            output += (
                f"【名前】{row['name']}【計】:{round(row['total_pl'], 0)}({row['positive_count']}:{row['negative_count']})"
    ...
    def execute_position_finish(self, cur_class, cur_row, cur_row_index):
        ...
        if (
            cur_row["low"] + adjuster < tp_price < cur_row["high"] + adjuster
            and cur_class.position_is_live
        ):  # （ロスカット優先）
            print(
                "　　 ■利確します",
                cur_class.name,
        ...
        if (
            cur_class.position_keeping_time_sec > cur_class.position_timeout_sec
        ):  # and cur_class.unrealized_pl < 0:
            # 本番ではマイナス継続が1分続いた場合だが、ここではマイナスでありかつ時間が経過なので、ある程度ずれる。ただマイナスはほぼ変わりない。
            print(
                "    Trade解消(マイナス×時間)",
                cur_class.position_keeping_time_sec,
                "> 規定Sec",
            ...
            cur_class.settlement_price = cur_row[
                "close"
            ]  # ポジション解消価格（ここは暫定的にOpen価格
            cur_class.position_is_live = False
            cur_class.is_live = False
        ...
        if not cur_class.position_is_live:
            print(
                "　　   [結果表示]取得価格",
                cur_class.target_price,
                "決済価格",
                cur_class.settlement_price,
            )
            # ポジション解消時にTarget PriceとTP/LC priceでの損益がPLに格納されているので、これを格納する
            cur_class.realized_pl = pl * abs(
                cur_class.units
            )  # 含み損益の更新（Unitsをかけたもの）　マイナス値を持つ
            cur_class.realized_pl_per_units = (
                pl  # 含み損益（Unitsに依存しない数） マイナス値を持つ
            )
    ...
    def draw_graph(self):
        ...
        self.result_df["order_time"] = pd.to_datetime(
            self.result_df["order_time"]
        )  # 決心時間（少しずれるので見にくい？）
        self.result_df["take_time"] = pd.to_datetime(
            self.result_df["take_time"]
        )  # 決心時間（少しずれるので見にくい？）
        ...
        for _, row in self.result_df.iterrows():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        symbol="line-ew", size=7, color="black", line=dict(width=1)
                    ),
                    name="LC Price",
                    showlegend=False,
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        symbol="line-ew", size=7, color="black", line=dict(width=3)
                    ),
                    name="TP Price",
                    showlegend=False,
                )
...
def cal_str_time_gap(time_str_1, time_str_2):
    ...
    time1 = (
        time_str_1
        if isinstance(time_str_1, datetime.datetime)
        else str_to_time(time_str_1)
    )
    time2 = (
        time_str_2
        if isinstance(time_str_2, datetime.datetime)
        else str_to_time(time_str_2)
    )
...
def update_order_information_and_take_position(
    cur_order_class, cur_5s_row, cur_row_index
):
    """
    オーダーが約定するかを確認する関数
    ...
    if cur_order_class.watch_for_order_exe:
        # ウォッチが有効の場合(書き直す必要あり
        order_keeping_time_sec = cal_str_time_gap(
            cur_5s_row["time_jp"], cur_order_class.order_time
        )["gap"]
        ...
        if order_keeping_time_sec >= cur_order_class.order_timeout_sec:
            ...
            if (
                cur_5s_row["low"] + adjuster
                <= target_price
                < cur_5s_row["high"] + adjuster
            ):
                print(
                    "　　■取得しました",
                    cur_order_class.name,
                    cur_5s_row["time_jp"],
                    cur_5s_row["low"],
                    cur_5s_row["high"],
                    target_price,
...
def update_position_information(cur_class, cur_row, cur_row_index):
    ...
    # 経過秒を計算する
    position_keeping_time_sec = cal_str_time_gap(
        cur_row["time_jp"], cur_class.position_time
    )["gap"]
    ...
    cur_row[
        "close"
    ]  # 暫定としてクローズ価格を現在価格とする ★NowPriceで考えるため、LCやTP priceとは誤差が出る。
    # 現価＞ターゲットだと現価-ターゲットは正の値。買いポジの場合は＋）
    pl_use_close = round(
        (cur_row["close"] + adjuster - cur_class.target_price) * cur_class.direction, 3
    )  # 現価＞ターゲットだと現価-ターゲットは正の値。買いポジの場合は＋）
    pl_use_high = round(
        (cur_row["high"] + adjuster - cur_class.target_price) * cur_class.direction, 3
    )  # 現価＞ターゲットだと現価-ターゲットは正の値。買いポジの場合は＋）
    pl_use_low = round(
        (cur_row["low"] + adjuster - cur_class.target_price) * cur_class.direction, 3
    )  # 現価＞ターゲットだと現価-ターゲットは正の値。買いポジの場合は＋）
    ...
    cur_class.position_keeping_time_sec = (
        position_keeping_time_sec  # 所持継続時間の更新
    )
    cur_class.unrealized_pl = pl_use_close * abs(
        cur_class.units
    )  # 含み損益の更新（Unitsをかけたもの）　マイナス値を持つ
    cur_class.unrealized_pl_per_units = (
        pl_use_close  # 含み損益（Unitsに依存しない数） マイナス値を持つ
    )
    cur_class.pl = (
        cur_class.unrealized_pl
    )  # 途中でTest範囲が過ぎると、結果が入らなくなるため、この段階から入れておく
    cur_class.pl_per_units = (
        cur_class.unrealized_pl_per_units
    )  # 途中でTest範囲が過ぎると、結果が入らなくなるため、この段階から入れておく
    # print(pl * abs(cur_class.units), pl, cur_row['time_jp'])
    if cur_class.direction == 1:
    ...
    for i, lc_item in enumerate(cur_class.lc_change):
        ...
        if (
            "done" in lc_item
            or cur_class.position_keeping_time_sec <= lc_item["time_after"]
        ):
        ...
        if (
            cur_class.unrealized_pl_low
            < lc_item["trigger"]
            < cur_class.unrealized_pl_high
        ):
