...
def make_same_price_list_from_target_price(
    target_price, target_dir, peaks_all, same_price_range, is_recall
):
    """
    ...
    for i, item in enumerate(peaks_all_for_loop):
        ...
        if (
            add_oldest_peak
            and i == len(peaks_all_for_loop) - 1
            and len(same_price_list) != 0
        ):
        ...
        if (
            ...
            if counter == 0:
                if not is_recall:
                    ...
                    recall_result = make_same_price_list_from_target_price(
                        item["peak"], target_dir, peaks_all, same_price_range, True
                    )
            ...
            if item["direction"] == target_dir:
                ...
                # 通過したピーク情報を初期化する
                near_point_gap = 100
                near_break_count = near_fit_count = depth_break_count = (
                    depth_fit_count
                ) = depth_point_gap = 0
            ...
            if item["direction"] != target_dir:
                ...
                if peak_gap <= 0:
                    depth_break_count += (
                        1  # マイナスというより、LINEを突破している位置にあるカウント
                    )
                else:
                    depth_fit_count += 1
    ...
    if len(same_price_list) == 0:
        ...
        same_price_list = sorted(
            same_price_list, key=lambda x: x["time"], reverse=True
        )  # 念のため時間順に並び替え
        strength_info = cal_strength_of_same_price_list(
            same_price_list, peaks_all, target_price, target_dir
        )
...
def cal_strength_of_same_price_list(
    same_price_list, peaks, base_price, latest_direction
):
    """
    ラインの強度を判定する。
    ...
    # この時点で入力できるリターン値の設定
    return_dic["peak_strength_ave"] = (
        sum([item["peak_strength"] for item in same_price_list]) / len_of_list
    )
    ...
    return_dic["all_range_strong_line"] = sum(
        item["near_break_count"] for item in same_price_list
    )
    ...
    if latest_direction == -1:
        ...
        if (
            min_peak["peak"] - dependence_allowed_range_max_price > base_price
        ):  # もともと＋だったが、－に修正した
        ...
        if (
            max_peak["peak"] + dependence_allowed_range_max_price < base_price
        ):  # もともと－だったが、＋に修正し
    ...
    if len_of_list == 1:
        ...
        between_peaks_num = info[
            "between_peaks_num"
        ]  # 可読性向上（直近から、当該ピークまでのピーク数）
    ...
    #         # print(s6, "複数時　強強度", minus_counter, len(same_list))
        ...
        for i in range(
            len_of_list
        ):  # この比較はlen_of_same_price_listのため、最後の調査終了用peakは入らない
            if same_price_list[i]["near_point_gap"] < 0:
                minus_counter += 1  # マイナスはLINEを超えた回数
        # ↓↓保存用
        if minus_counter > len_of_list * 0.5:
    ...
    if len_of_list == 2:
        time_gap = gene.cal_str_time_gap(
            same_price_list[0]["time"], same_price_list[1]["time"]
        )["gap_abs"]
        time_gap_min = time_gap / 60
        if time_gap_min <= 15:
            return_dic["line_strength"] = 0.1
            return_dic["remark"] = (
                return_dic["remark"] + "だったが、SamePrice幅狭すぎでNG"
            )

    return return_dic


def yobi_cal_strength_of_same_price_list(
    same_price_list, peaks, base_price, latest_direction
):
    """
    ラインの強度を判定する。
    ...
    minus_counter = 0  # 初期
    line_strength = 0.01
    line_position_strength = (
        0  # 1の場合は、一番上か一番下を示す（一番下でも-1としない）
    )
    len_of_same_price_list = len(
        same_price_list
    )  # 最後に調査最終時刻を示すデータがあるため、実データは-1
    # 調査期間すべてを通じて、最上位（または最下位）のLINEかを確認する。
    all_range_strong_line = sum(
        item["near_break_count"] for item in same_price_list
    )  # 0の場合(LINE越が）通算通じて強いLine
    remark = ""
    # 平均値はall_range_strong_lineが０の場合、除算エラーとなるため、ここで計算しておく（０除算とならないような仕組み）
    peak_strength_ave = (
        0
        if len_of_same_price_list == 0
        else sum([item["peak_strength"] for item in same_price_list[:-1]])
        / len_of_same_price_list
    )
    ...
    return_dic["same_time_latest"] = (
        0 if len(same_price_list) == 0 else same_price_list[0]["time"]
    )
    return_dic["all_range_strong_line"] = (
        all_range_strong_line  # 0が強い(Break回数が０）
    )
...
def tilt_cal(peaks, target_direction):
    ...
    # ■TILTの判定を行う
    direction_integrity = (
        True  # 方向性が一致しているかどうか（下側は上昇、上側が下降がここでは正とする）
    )
    ...
    for target_num in range(4, 7):
        ...
        on_line_num = near_line_num = (
            0  # 線上にあるものもカウントし、よりフラッグ形状であることを示したい
        )
        # 各要素がその直線（0.05pipsの切片分だけ余裕をとる）より上にあるかを確認する
        for i, item in enumerate(target_peaks):
            # ■座標(a,b)を取得する
            a = gene.cal_str_time_gap(oldest_info["time"], item["time"])[
                "gap_abs"
            ]  # 時間差分
            ...
            # ■線の近くにあるか[判定]
            near_line_margin = (
                abs(target_peaks[0]["peak"] - min_or_max_info["peak"]) * 0.405
            )  # * 0.405がちょうどよさそう
        ...
        # ■累積(numごと）
        ans = {
            "tilt_line_level_each": tilt_line_level_each,
            "count": target_num,
            "tilt_pm": tilt_pm,
            "direction": d,
    ...
    true_num = sum(
        1 for item in each_line_info if item["tilt_line_level_each"] != 0
    )  # tilt_line_level_each = 0はTiltではない。それ以外がTrue同等
    ...
    if (
        ...
        temp = next(
            (item for item in each_line_info if item["on_line_ratio"] == 1), None
        )
    ...
    first_item = next(
        (item for item in each_line_info if item["tilt_line_level_each"]), None
    )  # 最もLatestなTiltTrue
    # oldest_item = next((item for item in reversed(each_line_info) if item["tilt_line_level_each"]), None)  # 最もOldestなTiltTrue
    oldest_item = max(
        (
            item for item in reversed(each_line_info) if item["tilt_line_level_each"]
        ),  # fが真の要素をフィルタ
    ...
    lc_range = (
        temp_lc_price - now_price
    )  # これがマイナス値の場合Directionは１、プラス値となる場合Directionは-1
...
def judge_flag_figure(peaks, target_direction, df_r):
    ...
    if tilt_line_info["tilt_line_level"] > 0:
        ...
        if tilt_ans_prev["tilt_line_level"] > 0 and (
            target_direction == peaks_prev[0]["direction"]
        ):
    ...
    return {
        ...
        "is_first": is_first,
        "lc_price": tilt_line_info[
            "lc_price"
        ],  # こっちのほうがlongよりLC幅が狭いので。いつかリスクを追う場合は、Longに？）
...
def analysis(peaksclass):
    ...
    target_dir = peaks[0][
        "direction"
    ]  # Lineの方向 予測ではLatest。値が1の場合UpperLine（＝上値抵抗）
    grid = dependence_search_grid  # 調査の細かさ
    target_df.iloc[0]["time_jp"]
    ...
    max_index, max_peak_info_in_latest4 = max(
        enumerate(peaks[:min_max_search_range]), key=lambda x: x[1]["peak"]
    )
    min_index, min_peak_info_in_latest4 = min(
        enumerate(peaks[:min_max_search_range]), key=lambda x: x[1]["peak"]
    )
    if target_dir == 1:
        # latestが登り方向の場合
        search_max_price = (
            max_peak_info_in_latest4["peak"] + grid
        )  # 探索する最高値。基本こっちを利用
        ...
        search_min_price = (
            min_peak_info_in_latest4["peak"] - grid
        )  # 探索する最低値。基本こっちを利用。
    ...
    while search_min_price <= target_price <= search_max_price:
        ...
        same_price_info = make_same_price_list_from_target_price(
            target_price, target_dir, peaks, same_price_range, False
        )  # ★★
        if len(same_price_info["same_price_list"]) != 0:
            print(
                s6, same_price_info["same_price_list"]
            )  # ターゲットを基準にした、同一価格リスト（＆その強度）
            same_price_info["same_price_list"] = [
                d
                for d in same_price_info["same_price_list"]
                if d["time"] != peaks[0]["time"]
            ]  # Latestは削除する(predict特有）
            # 傾きの検証を行う（Flag形状）
            strength_info = same_price_info["strength_info"]  # コード短縮のための置換
            if (
                strength_info["line_on_num"] >= 2
                and strength_info["line_strength"] >= 0.9
            ):
                ...
                if flag_info["tilt_line_level"] > 0:
                    ...
                    same_price_info["strength_info"]["priority"] = (
                        3  # これは入れる場所検討
                    )
                    same_price_info["strength_info"]["expected_direction"] = peaks[0][
                        "direction"
                    ]  # 念のため。。。（これはここに依存しないようにしたいが）
                    ...
                    if dependence_line_close_border > gap_line_latest_price:
                        ...
                        is_close = False
                    same_price_info["strength_info"]["line_is_close_for_flag"] = (
                        is_close
                    )
        ...
        # ■■次のループへの処理
        skipper = (
            dependence_search_skipper
            if len(same_price_info["same_price_list"]) >= 1
            else 0
        )  # スキッパーの設定
        grid_adj = grid + skipper  # skipperをあらかじめgridに付加しておく
        target_price = target_price - (
            grid_adj * target_dir
        )  # 次のループへ（トップ⇒ボトムの検索。ボトム⇒トップの場合は*-1)
    ...
    if flag_flag:
        ...
        if flag_flag:
            ...
            latest_flag_info = max(
                filtered_flags,
                key=lambda x: max(detail["time"] for detail in x["same_price_list"]),
            )
...
def main_flag(peaks_class):
    ...
    if peaks[0]["count"] != 2:  #  not(2 <= peaks[0]['count'] < 4):
        print(
            s6, "countで実行しない", peaks[0]["count"]
        )  # フラッグは基本的に、Latestの数がいつでも実行する
        return orders_and_evidence
    else:
    ...
    lc_change = [
        ...
        # 2022-2023は 0.05トリガーにすると、マイナスになる！！
        {
            "lc_change_exe": True,
            "time_after": 0,
            "lc_trigger_range": 0.06,
            "lc_ensure_range": 0.04,
        },
    ]
    ...
    if flag_info["strength_info"]["line_is_close_for_flag"]:
        ...
        # 初回ではない場合(オーダー）　⇒突破のみ
        if (
            flag_info["flag_info"]["tilt_line_level"] >= 1
        ):  # フラッグ形状がきれい（＝１）場合、突破
            break_position = True
        else:
        ...
        if break_position:
            ...
            dependence_lc_range_at_least_c = (
                0.022  # 0.12が強い　カウンターはそもそもが適当なため、狭いめ
            )

            # ■■フラッグ用（突破方向）
            # 最大でもLCRange換算で10pips以内したい
            target_price = flag_info["line_base_info"]["line_base_price"] + (
                dependence_normal_margin
                * flag_info["line_base_info"]["line_base_direction"]
            )  # lcPriceは収束の中間点
            temp_lc_price = flag_info["flag_info"]["lc_price"]
            lc_range = (
                temp_lc_price - target_price
            )  # これがマイナス値の場合Directionは１、プラス値となる場合Directionは-1
            ...
            if abs(lc_range) >= dependence_lc_range_max:
                ...
                if lc_range < 0:
                    ...
                    print(
                        ...
                        target_price,
                        "-",
                        abs(dependence_lc_range_at_least),
                    )
                else:
                    # LC_rangeがプラス値　＝　Directionは-1。その為、現在書くにプラスするとＬＣＰｒｉｃｅになる
                    lc_price = target_price + abs(dependence_lc_range_at_least)
            ...
            main_order_base = cf.order_base(
                df_r.iloc[0]["close"], df_r.iloc[0]["time_jp"]
            )  # tpはLCChange任せのため、Baseのまま
            ...
            main_order_base["expected_direction"] = flag_info["strength_info"][
                "expected_direction"
            ]
            ...
            main_order_base["name"] = (
                "フラッグ強" + flag_info["strength_info"]["remark"]
            )
            ...
            if abs(lc_range) >= dependence_lc_range_max_c:
                ...
                if lc_range < 0:
                    ...
                    lc_price_counter = target_price + abs(
                        dependence_lc_range_max_c
                    )  # 通常とは符号逆 もともとー
                    print(
                        s6,
                        "LC価格(Dir1)",
                ...
                if lc_range < 0:
                    ...
                    print(
                        ...
                        target_price,
                        "-",
                        abs(dependence_lc_range_at_least_c),
                    )
                else:
                    # LC_rangeがプラス値　＝　買い注文。その為、ターゲット価格からマイナスするとＬＣＰｒｉｃｅになる
                    lc_price_counter = target_price - abs(
            ...
            main_order_base["type"] = position_type
            main_order_base["expected_direction"] = (
                flag_info["strength_info"]["expected_direction"] * -1
            )
            main_order_base["priority"] = 5  # ['strength_info']['priority']
            main_order_base["units"] = main_order_base["units"] * 1
            main_order_base["lc_change"] = lc_change
            main_order_base["name"] = (
                "フラッグ強リバース" + flag_info["strength_info"]["remark"]
            )
            main_order_base["y_change"] = flag_info["strength_info"]["y_change"]
            exe_orders.append(cf.order_finalize(main_order_base))
        else:
            ...
            # 最大でもLCRange換算で10pips以内したい
            target_price = (
                flag_info["line_base_info"]["line_base_price"]
                + (
                    dependence_normal_margin
                    * flag_info["line_base_info"]["line_base_direction"]
                )
            ...
            main_order_base["units"] = main_order_base["units"] * 1
            main_order_base["name"] = (
                "フラッグ弱(リバース)" + flag_info["strength_info"]["remark"]
            )
            main_order_base["y_change"] = flag_info["strength_info"]["y_change"]
            exe_orders.append(cf.order_finalize(main_order_base))
            ...
            main_order_base["name"] = (
                "フラッグ弱(ブレイク)"
                + flag_info["strength_info"]["remark"]
                + "(count:"
                + str(peaks[0]["count"])
                + ")"
            )
