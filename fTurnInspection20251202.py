...
class MainAnalysis:
                ...
                if foot == "M5":
                    ...
                    base_dic = [
                        ...
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(1.8),
                            "ensure": ca.cal_move_ave(1.2),
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(2.5),
                            "ensure": ca.cal_move_ave(1.8),
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(3.3),
                            "ensure": ca.cal_move_ave(2.6),
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(4.3),
                            "ensure": ca.cal_move_ave(3.6),
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(5.3),
                            "ensure": ca.cal_move_ave(4.6),
                        },
                        {
                            "exe": True,
                            "time_after": 600,
                            "trigger": ca.cal_move_ave(6.3),
                            "ensure": ca.cal_move_ave(5.6),
                        },
                    ]
                elif foot == "H1":
                    print(s, s, "foot指定有 H1@lc_change")
    ...
    def main(self):
        ...
        is_same = (gl_previous_exe_df60_row is not None) and df_h1_row.equals(
            gl_previous_exe_df60_row
        )
        if is_same:
            pass
            print(
                "1時間足のデータが同じだった ⇒ これは5分毎で値の変わらない検証で多く起こる"
            )
            self.bb_h1_class = gl_previous_bb_h1_class  # クラスは新規生成されるため、グローバル変数で記録しておく
        else:
            print("データが異なるので調査対象")
            if (
                gl_previous_exe_df60_order_time
                != candle_analysis.d60_df_r.iloc[0]["time_jp"]
            ):
                print(
                    "まだオーダーをしていない時間のため調査対象",
                    gl_previous_exe_df60_order_time,
                    candle_analysis.d60_df_r.iloc[0]["time_jp"],
                )
                gl_previous_bb_h1_class = BbAnalysis(
                    candle_analysis, "H1", mode
                )  # クラスは新規生成されるため、グローバル変数で記録しておく
                ...
                if gl_previous_bb_h1_class.take_position_flag:
                    self.add_order_to_this_class(
                        gl_previous_bb_h1_class.exe_order_classes
                    )  # オーダーを登録
                    gl_previous_exe_df60_order_time = (
                        gl_previous_bb_h1_class.latest_time_60
                    )  # １H起点のオーダーを入れた時刻を取得（重複オーダー防止用）
            else:
                print(
                    "オーダー済みの時刻",
                    gl_previous_exe_df60_order_time,
    ...
    def predict_registance_turn_analysis(self):
        ...
        # 狭すぎるレンジでは実行しない
        if (
            turn_line_info["same_price_list_till_break_5_total"] >= 10
            and flop_line_info["same_price_list_till_break_5_total"] >= 10
            and abs(
                peaks[1]["latest_body_peak_price"] - peaks[0]["latest_body_peak_price"]
            )
            <= 0.1
        ):
            print(
                s,
                "turn抵抗線とflop抵抗線が強く、その幅が狭い（極めて狭いレンジ）のでNG) ",
                turn_line_info["same_price_list_till_break_5_total"],
                flop_line_info["same_price_list_till_break_5_total"],
                abs(
                    peaks[1]["latest_body_peak_price"]
...
class predict_turn_analysis:
    def __init__(self, candle_analysis):
        ...
        self.lc_change_test = [
            ...
            },  # ←とにかく、LCCandleを発動させたい場合
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(2),
                "ensure": -0.001,
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(4),
                "ensure": self.ca5.cal_move_ave(2),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(5),
                "ensure": self.ca5.cal_move_ave(3),
            },
            {
                "exe": True,
                "time_after": 6000,
                "trigger": self.ca5.cal_move_ave(6),
                "ensure": self.ca5.cal_move_ave(4),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(7),
                "ensure": self.ca5.cal_move_ave(5),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(8),
                "ensure": self.ca5.cal_move_ave(6),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(9),
                "ensure": self.ca5.cal_move_ave(7),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(10),
                "ensure": self.ca5.cal_move_ave(8),
            },
            {
                "exe": True,
                "time_after": 600,
                "trigger": self.ca5.cal_move_ave(11),
                "ensure": self.ca5.cal_move_ave(9),
            },
        ]
    ...
    def predict_lines_detect(self, peaks_class, name):
        ...
        if not latest_dir_lines["comment"] == "" or not turn_dir_lines["comment"] == "":
            matome = (
                ...
                + str(latest_d)
                + "\n"
                + latest_dir_lines["comment_analy"]
                + "Turn側\n"
        ...
        return {"exist": exist, "comment": comment}
    ...
    def predict_lines_detect_each(self, peaks_class, direction):
        ...
        if direction == 1:
            ...
            if latest_d == 1:
                result = [
                    d
                    for d in peaks
                    if d["direction"] == latest_d
                    and d["latest_body_peak_price"] > base_price
                ]
                result = sorted(
                    result, key=lambda x: x["latest_body_peak_price"], reverse=False
                )  # Falseで近い順
            else:
                result = [
                    d
                ...
                result = sorted(
                    result, key=lambda x: x["latest_body_peak_price"], reverse=True
                )  # Trueで近い順　Falseがもともと
        else:
        ...
        for i, item in enumerate(result):
            line_info = self.pickup_same_price_list(
                peaks_class, item["i"]
            )  # flop部分もの（上下かは問わず）
        ...
        for i, item in enumerate(merged_strong_lines["merged_line_lists"]):
            ...
            analy_t = (
                analy_t
                + str(item["lines_strength"])
                + ", "
                + str(item["gap_latest_price"])
                + ", "
        ...
        t = ""
        if (
            not merged_strong_lines["comment"] == ""
            or not merged_week_lines["comment"] == ""
        ):
            # print("どっちがあるため表示")
            ...
            t = (
                t
                + "\n「強」"
                + str(len(merged_strong_lines["merged_line_lists"]))
                + "\n"
            ...
            t = (
                t
                + "\n「弱」"
                + str(len(merged_week_lines["merged_line_lists"]))
                + "\n"
    ...
    def pickup_same_price_list(self, peaks_class, target_no):
        ...
        same_price_list_till_break_total = sum(
            d["item"]["peak_strength"] for d in same_price_list_till_break
        )
        # 方向
        d = same_price_list[0]["item"]["direction"]
        ...
        # プリント用のメッセージを準備しておく
        target_price = str(
            peaks_with_same_price_list[target_no]["latest_body_peak_price"]
        )
        str(peaks_with_same_price_list[target_no]["latest_time_jp"])
    ...
    def merge_lines(self, peaks_class, same_price_lists, same_range):
        ...
        if len(same_price_list) == 0:
            return {"merged_line_lists": [], "comment": ""}
        ...
        for i, item in enumerate(same_price_list):
            ...
            if i == 0:
                ...
                # 初回以外は通常処理
                if (
                    abs(base_price - same_price_list_head["latest_body_peak_price"])
                    <= same_range
                ):
                    # 同一価格が見つかった場合
                    if base_info == item:
        ...
        for i, item in enumerate(f):
            ...
            gap_latest_price = round(
                abs(latest_price - item[0]["item"]["latest_body_peak_price"]),
                self.round_digit,
            )
            ...
            for m, mtem in enumerate(item):
                d = int(
                    mtem["item"]["direction"]
                )  # LINE送付用に取得（毎回取得するが全部同じなのでいいや）
                time_jp = gene.str_to_time_short(mtem["item"]["latest_time_jp"])
                com = (
                    com
                    + str(mtem["item"]["latest_body_peak_price"])
                    + "("
                    + time_jp
                    + "),"
                )
        ...
        return {"merged_line_lists": final_merged_lists, "comment": com_all}
    ...
    def bb_range_analysis(self, df_r):
        ...
        df_r.iloc[check_point1]["bb_range"]
        df_r.iloc[check_point2]["bb_range"]
        df_r.iloc[check_point3]["bb_range"]
        print(
            self.s,
            df_r.iloc[check_point1]["time_jp"],
            df_r.iloc[check_point1]["bb_range"],
            df_r.iloc[check_point1]["bb_upper"],
        )
        print(
            self.s,
            df_r.iloc[check_point2]["time_jp"],
            df_r.iloc[check_point2]["bb_range"],
            df_r.iloc[check_point2]["bb_upper"],
        )
        print(
            self.s,
            df_r.iloc[check_point3]["time_jp"],
            df_r.iloc[check_point3]["bb_range"],
            df_r.iloc[check_point3]["bb_upper"],
        )
        ...
        self.bb_current_ratio = (
            100 * (self.bb_upper - self.current_price) / (self.bb_upper - self.bb_lower)
        )
        # 現在の位置関係
        ...
        (
            100
            * (self.bb_upper - self.latest_peak_price)
        ...
        # 幅が同じでも、移動している場合があるため、三つのラップ率を算出する（直近のBBに対して、何割ラップしているか）
        pairs = [
            (df_r.iloc[check_point1]["bb_lower"], df_r.iloc[check_point1]["bb_upper"]),
        ...
        if (
            rap_res == 0
        ):  # 結構ラップしている場合(中央の価格は変わっていない⇒後はすぼんでるか広がってるか）
            pass

        comment_all = (
            res_com + "　:　" + "【" + order_recommend + "】" + str(ts) + ",　" + str(t)
        )
