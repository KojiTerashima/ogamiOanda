...
class PeaksClass:
    def __init__(self, original_df_r, granularity, notifier: Notifier | None = None):
        ...
        self.data_hold_peaks = (
            3  # 容量の関係で、直近のNピーク分のみ、データフレームも持つ。
        )
        ...
        self.ps_most_max = (
            8  # 強いピークとみなす（直近数時間で最も高い（または低い）ピークの場合)
        )
        ...
        if granularity == "M5":
            ...
            self.fluctuation_gap = (
                0.3  # 急変動とみなす1足の変動は30pips以上。（1足でPeakの変動ではない）
            )
            self.fluctuation_count = (
                3  # 3カウント以下でfluctuation_gapが起きた場合、急変動とみなす
            )
            # 抵抗線関係の値　cal_big_mountain関数
            self.arrowed_gap = 0.017  # 0.0245  # 抵抗線を探す時、ずれていてもいい許容値
            self.arrowed_break_gap = (
                0.02  # 抵抗線を探す時、これ以上越えていると、Breakしてると判断する範囲
            )
            # 狭いレンジの期間の判定用の閾値
            self.check_very_narrow_range_range = (
                0.07  # 一つの足のサイズが、この閾値以下の場合、小さいレンジの可能性
            )
            # 大きな動きのクライテリア
            ...
            self.arrowed_break_gap = (
                0.04  # 抵抗線を探す時、これ以上越えていると、Breakしてると判断する範囲
            )
            # 狭いレンジの期間の判定用の閾値
            self.check_very_narrow_range_range = (
                0.07  # 一つの足のサイズが、この閾値以下の場合、小さいレンジの可能性
        ...
        # 基本となるデータフレーム
        self.df_r_original = (
            None  # 直近の時刻が[0]となるデータフレーム。API取得のデータとは逆順
        )
        # ピーク情報
        ...
        self.gap_price_and_latest_turn_peak_abs = abs(
            self.latest_peak_price - self.latest_price
        )
        # 動きの数値化された情報
        self.is_big_move_peak = False
        self.is_big_move_candle = False
        self.ave_move = 0  # ここ5足程度の動きの大きさ（髭を含む）
        self.ave_move_for_lc = (
            0  # ここ5足程度の動きの大きさ（髭を含む）を加味した、LCの提案価格
        )
        # 時間情報
        self.time_hour = 0
        # samePriceList関係
        ...
        self.df_r = self.df_r[
            : self.analysis_num
        ]  # 直近4.5時間分(55足分)のデータフレームにする
        temp_res = self.make_peaks(
            self.df_r
        )  # 一番感度のいいPeaks。引数は書くとするなら。self.df_r。
        self.peaks_original = temp_res[
            "peaks"
        ]  # self.make_peaks(self.df_r)  # 一番感度のいいPeaks。引数は書くとするなら。self.df_r。
        ...
        self.gap_price_and_latest_turn_peak_abs = abs(
            self.latest_price - self.latest_peak_price
        )

        # (3) 時間の取得
        time_obj = pd.to_datetime(
            original_df_r.iloc[0]["time_jp"], format="%Y/%m/%d %H:%M:%S"
        )
    ...
    def make_peak(self, df_r):
        ...
        for i in range(len(data_df) - 1):
            ...
            # ■カウントを進めていく
            if (
                tilt_direction == base_direction
            ):  # 今回の検証変化率が、初回と動きの方向が同じ場合
                counter += 1
            else:
        ...
        gap_close = round(
            abs(latest_body_price - ans_df.iloc[-1]["close"]), self.round_keta
        )  # 直近の価格（クローズの価格&向き不問）
        if gap_close == 0:
            gap_close = self.minimum
        else:
        ...
        gap = round(
            abs(latest_body_price - oldest_body_price), self.round_keta
        )  # 直近の価格（クローズの価格&向き不問）
        if gap == 0:
            gap = self.minimum
        else:
        ...
        gap_high_low = round(
            abs(latest_wick_price - oldest_wick_price), self.round_keta
        )  # 直近の価格（クローズの価格&向き不問）
        if gap_high_low == 0:
            gap_high_low = self.minimum
        else:
        ...
        ans_dic["data"] = (
            ans_df  # 対象となるデータフレーム（元のデータフレームではない）
        )
        ...
        ans_dic["memo_time"] = (
            f.str_to_time_hms(ans_df.iloc[-1]["time_jp"])
            + "_"
            + f.str_to_time_hms(ans_df.iloc[0]["time_jp"])
        )  # 表示に利用する時刻表示用
        # 追加項目
        ans_dic["include_large"] = self.check_large_body_in_peak(ans_dic)[
            "include_large"
        ]
        ans_dic["include_very_large"] = self.check_large_body_in_peak(ans_dic)[
            "include_very_large"
        ]
        ...
        ans_dic["peak"] = ans_dic[
            "latest_body_peak_price"
        ]  # ピークのボディ価格（Latest。方向ごと）
        ans_dic["time_old"] = ans_dic["oldest_time_jp"]  #
        ans_dic["peak_old"] = ans_dic[
            "oldest_body_peak_price"
        ]  # 重複するが、わかりやすい名前で持っておく
        # 返却する
        return ans_dic
    ...
    def make_peaks(self, df_r):
        ...
        for i in range(222):
            ...
            if (
                len(peaks) != 0
                and peaks[-1]["latest_time_jp"] == this_peak["latest_time_jp"]
            ):
            ...
            if (
                i <= self.data_hold_peaks
            ):  # 容量の関係で、最初の10ピーク分のみ、DFを入れる
                ...
                # ]
                this_peak_copy = copy.deepcopy(
                    this_peak
                )  # コピーを取る（参照先のthis_peaks自体は後で変更するため）
                this_peak_copy["data"] = this_peak_copy["data"][
                    [c for c in cols_to_keep if c in this_peak_copy["data"].columns]
                ]
            ...
            this_peak.pop(
                "data", None
            )  # DataFrameを削除する# 存在しない場合はエラーを防ぐためにデフォルト値を指定
            this_peak.pop("data_remain", None)  # DataFrameを削除する
            # peakのコピーを生成（PreviousやNextがない状態）
            this_peak_simple_copy = (
                this_peak.copy()
            )  # 処理的に次（時間的には後）のピークとして保管
            # 後ピークの追加（時間的に後）
            this_peak["next"] = next_time_peak
            next_time_peak = (
                this_peak_simple_copy  # 処理的に次（時間的には後）のピークとして保管
            )
            # 前関係の追加 (現在処理⇒Peak,ひとつ前の処理（時間的には次）はpeaks[-1])
            if i != 0:
                # 先頭以外の時(next_timeはある状態。previousは次の処理で取得される）。先頭の時は実施しない。
                peaks[-1]["previous_time_peak"] = (
                    this_peak_simple_copy  # next_timeのpreviousに今回のpeakを追加
                )
        ...
        for i, item in enumerate(peaks):
            ...
            if item["gap"] <= self.peak_strength_border or (
                item["gap"] <= self.peak_strength_border_second
                and item["count"] <= count_border
            ):
                if item["gap"] <= self.peak_strength_border_min:
                    # ほぼ見えないレベルの折り返しは点数を最も低くする
                    item["peak_strength"] = (
                        self.ps_most_most_min
                    )  # これで元データ入れ替えられるんだ？！
                    peaks[i + 1]["peak_strength"] = (
                        self.ps_most_most_min
                    )  # ひとつ前(時間的はOldest）のPeakも強制的にStrengthが1となる
            ...
            overlap_ratio = (
                0.4  # ラップ率のボーダー値　(0.7以上でラップ大。0.7以下でラップ小）
            )
            if (
                item_latest_ratio <= overlap_ratio
                and item_oldest_ratio <= overlap_ratio
            ):
                # このアイテムのGapが小さい場合、直前も低くなる事に注意
    ...
    def peak_strength_sort(self):
        ...
        # print("1側")
        # gene.print_arr(sorted_peaks_by_dir[1])
        return {"-1": sorted_peaks_by_dir[-1], "1": sorted_peaks_by_dir[1]}
    ...
    def skip_peaks(self):
        ...
        peaks = copy.deepcopy(
            self.peaks_original
        )  # PeaksClass.peaks_original.copy()では浅いコピーとなる
        # # フラグつけ用
        ...
        while (
            i < len(peaks) - 1
        ):  # 中で配列を買い替えるため、for i in peaksは使えない！！
            ...
            count_border = 2
            if (
                vanish_item["count"] <= count_border
                and vanish_item["gap"] <= self.skip_gap_border
            ):
                pass
            ...
            # 判定１
            overlap_ratio = (
                0.6  # ラップ率のボーダー値　(0.7以上でラップ大。0.7以下でラップ小）
            )
            ...
            if (
                vanish_latest_ratio >= overlap_ratio
                and vanish_oldest_ratio >= overlap_ratio
            ):
                ...
                if vanish_item["count"] <= count_border:
                    if (
                        vanish_latest_ratio <= overlap_min_ratio
                        and vanish_oldest_ratio <= overlap_ratio
                    ):
                        # print("　latestに対して、Vanishが小さく、ラップが小さいほうがあるため、SKIP")
                        is_skip = True
                    elif (
                        vanish_oldest_ratio <= overlap_min_ratio
                        and vanish_latest_ratio <= overlap_ratio
                    ):
            ...
            if is_skip:
                if "previous" in oldest_merged_item:
                    ...
                    skip_counter = (
                        skip_counter + 2
                    )  # 2個消すので、2個分インクリメントが必要

                    # 消す作業の本番
                    peaks[latest_num]["oldest_body_peak_price"] = oldest_merged_item[
                        "oldest_body_peak_price"
                    ]
                    peaks[latest_num]["oldest_time_jp"] = oldest_merged_item[
                        "oldest_time_jp"
                    ]
                    peaks[latest_num]["oldest_price"] = oldest_merged_item[
                        "oldest_price"
                    ]
                    peaks[latest_num]["count"] = (
                        latest_item["count"]
                        + vanish_item["count"]
                        + oldest_merged_item["count"]
                    ) - 2
                    peaks[latest_num]["previous"] = oldest_merged_item["previous"]
                    peaks[latest_num]["gap"] = round(
                        abs(
                    ...
                    peaks[latest_num]["peak_strength"] = (
                        self.ps_default
                    )  # 従来はlatest_item['strength']だが、結合＝０にした方がよい場合が、、
                    peaks[latest_num]["skip_include_num"] = (
                        peaks[latest_num]["skip_include_num"] + 1
                    )
                    # 互換性確保用（いつか消したい）
    ...
    def skip_peaks_hard(self):
        ...
        peaks = copy.deepcopy(
            self.skipped_peaks
        )  # self.peaks_original.copy()では浅いコピーとなる
        ...
        while (
            ...
            if is_skip:
                ...
                if "previous" in oldest_item:
                    ...
                    # 消す作業の本番
                    peaks[latest_num]["oldest_body_peak_price"] = oldest_item[
                        "oldest_body_peak_price"
                    ]
    ...
    def temp(self):
        ...
        peaks_same_direction = [
            d for d in self.peaks_original if d["direction"] == target_peak["direction"]
        ]
        [
            d
            for d in self.peaks_original
            if d["direction"] == target_peak["direction"] * -1
        ]
        target_peak["peak"]
        ...
        closest_element = min(
            (
                d for d in peaks_same_direction if d["peak"] != target_peak["peak"]
            ),  # 自分自身を除外
    ...
    def make_same_price_list(self, target_num, skip):
        ...
        # 山の情報
        mountain_foot_min = (
            60  # 山のすそ野の広さ 足の個数（この値以上の山の裾野の広さを狙う）
        )
        ...
        same_price_num = 0
        strength_border = (
            1  # この点数以下のストレングスはカウントしない（１点はしない）
        )
        ...
        for i, item in enumerate(peaks):
            ...
            time_gap_sec = abs(
                datetime.strptime(item["latest_time_jp"], "%Y/%m/%d %H:%M:%S")
                - base_time
            )
            ...
            if item["direction"] != target_peak["direction"]:
                if i == 0:
                    ...
                    self.opposite_peaks.append(
                        {"i": i, "item": item, "time_gap": time_gap_sec}
                    )
                    # print("             反対側")
                continue
            ...
            if i == 0:  # 時系列的に過去のもののみがPeaksにある場合はこっち。
                ...
                self.same_price_list.append(
                    {"i": i, "item": item, "time_gap": time_gap_sec}
                )
                self.same_price_list_till_break.append(
                    {"i": i, "item": item, "time_gap": time_gap_sec}
                )
                self.same_price_list_till_break2.append(
                    {"i": i, "item": item, "time_gap": time_gap_sec}
                )
                if is_inner:
                    self.same_price_list_inner.append(
                        {"i": i, "item": item, "time_gap": time_gap_sec}
                    )
                else:
                    self.same_price_list_outer.append(
                        {"i": i, "item": item, "time_gap": time_gap_sec}
                    )
                continue
            ...
            if target_peak["direction"] == 1:
                if item["peak"] > target_price + self.arrowed_break_gap:
                    ...
                    self.break_peaks.append(
                        {"i": i, "item": item, "time_gap": time_gap_sec}
                    )
                    if is_inner:
                        self.break_peaks_inner.append(
                            {"i": i, "item": item, "time_gap": time_gap_sec}
                        )
                    else:
                        self.break_peaks_outer.append(
                            {"i": i, "item": item, "time_gap": time_gap_sec}
                        )
                    # 共通
            ...
            if abs(item["latest_wick_peak_price"] - item["peak"]) >= arrowed_range:
                ...
                arrowed_range + 1  # 確実にarrowed_rangeより大きな値
            else:
                abs(target_price - item["latest_wick_peak_price"])
            ...
            if body_gap_abs <= arrowed_range:  # or body_wick_gap_abs <= arrowed_range:
                ...
                self.result_not_same_price_list.append(
                    {"i": i, "item": item, "time_gap": time_gap_sec}
                )
        # 表示用
        # print("同一価格一覧 @cp")
    ...
    def check_large_body_in_peak(self, block_ans):
        ...
        # 足や通過に依存する数字
        # 情報を取得する
        sorted_df_by_body_size = block_ans["data"].sort_values(
            by="body_abs", ascending=False
        )
        ...
        for index, row in sorted_df_by_body_size.iterrows():
            ...
            row["body_abs"] if row["body_abs"] != 0 else self.minimum
...
def judge_peak_is_belong_peak_group(peaks, target_peak):
    ...
    if target_peak["direction"] == 1:
        if (
            max_peak["peak"] - depend_judge_range
            <= target_peak["peak"]
            <= max_peak["peak"] + depend_judge_range
        ):
            print("最大群")
        ...
        if (
            min_peak["peak"] - depend_judge_range
            <= target_peak["peak"]
            <= min_peak["peak"] + depend_judge_range
        ):
            print("最小群")
