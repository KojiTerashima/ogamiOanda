...
class order_information:
    ...
    def __init__(self, name, oa):
        ...
        self.crcdo_sec_counter = (
            0  # ポジション所有から何秒後に、最新のCRCDOを行ったかを記録する。
        )
        self.crcdo_border = 0  # ロスカや利確を変更するライン
        self.crcdo_guarantee = (
            0  # 最低限確保する利益を指定（マイナスだとマイナス範囲でせり上げる）
        )
        ...
        self.crcdo_history_by_time = (
            False  # 時間経過によるポジショん変更履歴があるかどうか
        )
        ...
        # リセット対象外（規定値がある物）
        self.order_timeout = (
            20  # リセ無。分で指定。この時間が過ぎたらオーダーをキャンセルする
        )
    ...
    def reset(self):
        ...
        self.api_try_num = (
            3  # APIのエラー（今回はLC底上げに利用）は３回までself.crcdo_history = False
        )
        self.crcdo_sec_counter = (
            0  # ポジション所有から何秒後に、最新のCRCDOを行ったかを記録する。
        )
    ...
    def print_i(self):
        oa.print_i(
            "   <表示>", self.name, datetime.datetime.now().replace(microsecond=0)
        )
    ...
    def plan_info_input(
        self, info
    ):  # plan_info を更新する＋過去の情報を保存しておく(plan_input_and_judge_newからのみ呼ばれる）
        # ■初回は何も入っていない
    ...
    def input_info_and_judge_new(self, new_info):  # accept_new_order
        ...
        wait_time_new = (
            5  # ６分以上で、上書きオーダーの受け入れが可能のフラグを出せる。
        )
        if (
            0 < self.order["time_past_continue"] < wait_time_new * 59
        ):  # 0の場合はTrueなるので、不等号に＝はNG。オーダー発行時比較
            # print(" □発行直後のオーダー等、発行できない理由あり")
            print("時間不可")
            new_order = False
        else:
            self.plan_info_input(
                new_info
            )  # 時間的に問題なければ、入れておく（2023時点、ひとつ前の山と比較するため、必要）
    ...
    def make_order(self):
        ...
        if self.plan["price"] != 999:  # 例外時（price=999)以外は、通常通り実行する
            order_ans_dic = oa.OrderCreate_dic_exe(
                self.plan
            )  # Plan情報からオーダー発行しローカル変数に結果を格納する
            ...
            self.plan["tp_price"] = float(
                order_ans["tp_price"]
            )  # 念のため入れておく（元々計算で入れられるけど。。）
            self.plan["lc_price"] = float(
                order_ans["lc_price"]
            )  # 念のため入れておく（元々計算で入れられるけど。。）
            ...
            if order_ans["cancel"]:  # キャンセルされている場合は、リセットする
                ...
                tk.line_send(
                    " 　Order不成立（今後ループの可能性）", order_ans["order_id"]
                )
            else:
                print("      ↑オーダー発行完了")
    ...
    def close_position(self):
        ...
        if self.life:
            ...
            if res_dic["error"] == -1:
                ...
                self.life_set(False)
                tk.line_send(
                    "  ポジション解消", self.position["id"], self.position["pips"]
                )
        else:
            # print("    position無し")
    ...
    def update_information(self):  # orderとpositionを両方更新する
        ...
        if self.life:  # LifeがTrueの場合は、必ずorderIDが入っている
            ...
            if (
                self.order["state"] == "PENDING" and temp["order_state"] == "FILLED"
            ):  # 現orderあり⇒約定（取得時）
                ...
                self.order["state"] == "PENDING" and temp["position_state"] == "CLOSED"
            ):  # 現orderあり⇒ポジクローズ
                oa.print_i("  ★即ポジ即解消済！")
                self.life_set(False)
                tk.line_send(
                ...
                self.position["state"] == "OPEN" and temp["position_state"] == "CLOSED"
            ):  # 現ポジあり⇒ポジ無し（終了時）
                oa.print_i("  ★position解消")
                self.life_set(False)
                self.pips_res_arr.insert(
                    0, temp["position_pips"]
                )  # 先頭に、結果を追加していく（先頭からの方が後々集計しやすい）
                self.pips_res = temp["position_pips"]
                gl_total_pips = round(
                    gl_total_pips + temp["position_pips"], 3
                )  # トータル計算 abs(int(self.order['units']))
                temp_yen_result = int(
                    temp["position_pips"] * abs(self.order["units"])
                )  # トータルの円（plだと上手く表示されないので計算する）
                ...
                res2 = (
                    "【決済:"
                    + str(temp["position_close_price"])
                    + ", "
                    + "取得:"
                    + str(self.order["price"])
                    + "】"
                )
                res3 = (
                    "【ポジション期間の最大/小の振れ幅】 ＋域:"
                    + str(self.pips_lose_max)
                    + "/ー域:"
                    + str(self.pips_win_max)
                )
                res4 = (
                    "【今回結果】"
                    + str(temp["position_pips"])
                    + ","
                    + str(temp_yen_result)
                    + "円\n"
                )
                res5 = (
                    "【合計】計" + str(gl_total_pips) + ",計" + str(gl_total_yen) + "円"
                )
                now_time_only = oanda_class.str_to_time_hms(
                ...
                self.order["state"] == "PENDING" and temp["order_state"] == "CANCELLED"
            ):  # （取得時）
            ...
            self.order["time_str"] = temp[
                "order_time"
            ]  # 日時（日本）の文字列版（時間差を求める場合に利用する）
            self.order["time_past"] = int(
                temp["order_time_past"]
            )  # 諸事情でプラス２秒程度ある　経過時間を求める（Trueの場合）
            self.order["time_past_continue"] = oanda_class.cal_past_time_single(
                self.order["time_str"]
            )
            # ↑ 引数は元データ(文字列時刻)。オーダーを解除しても継続してカウントする秒数
            self.order["units"] = int(temp["order_units"])
            self.order["price"] = float(
                temp["order_price"]
            )  # これは入れ替えない方がよい？入れ替える必要がない？
            ...
            self.position["time_past"] = int(
                temp["position_time_past"]
            )  # 諸事情でプラス２秒程度ある
            self.position["time_past_continue"] = oanda_class.cal_past_time_single(
                self.position["time_str"]
            )
            ...
            # 時間による解消を行う
            if (
                self.order["time_past"] > self.order_timeout * 60
                and self.order["state"] == "PENDING"
            ):
                print("   時間解消@")
    ...
    def lc_change(
        ...
        if (
            self.crcdo_history is False and self.position["state"] == "OPEN"
        ):  # ポジションのCRCDO歴がない場合
            if (
                p["time_past"] - self.crcdo_sec_counter > 60
            ):  # N秒以上経過している場合、ロスカ引き上げ
                # （１）ある程度プラスになった場合、LCを引き上げて最低限のプラスを確保する（微マイナスでとどめる場合もあり）
                if (
                    p["pips"] > self.crcdo_border != 0
                ):  # crcdo_borderが０ではない（トレール有効）、borderより超えている場合
                    self.crcdo_lc_price = round(
                        p["price"] - guarantee
                        if self.plan["ask_bid"] < 0
                        else p["price"] + guarantee,
                        3,
                    )
                    data = {
                        "stopLoss": {
                            "price": str(self.crcdo_lc_price),
                            "timeInForce": "GTC",
                        },
    ...
    def lc_change_by_time(
        self,
    ):  # ポジションのLC底上げを実施 (最初の数分(秒単位で指定）はLC幅を広げておく、等で利用可能）
        p = self.position  # ポジションの情報
        o = self.order
        target_lc = self.crcdo_target_lc_by_time  # 変更後（狭いLC）

        if (
            p["time_past"] >= self.crcdo_border_by_time
        ):  # 規定時間の経過をしたら、LCを上げる
            if (
                self.crcdo_history_by_time is False
                and self.position["state"] == "OPEN"
                and self.crcdo_history is False
            ):  # ポジションのCRCDO歴がない場合
                crcdo_lc_price_o = round(
                    o["price"] - target_lc
                    if self.plan["ask_bid"] > 0
                    else o["price"] + target_lc,
                    3,
                )
                crcdo_lc_price_n = round(
                    p["price"] - target_lc
                    if self.plan["ask_bid"] > 0
                    else p["price"] + target_lc,
                    3,
                )
                print(
                    "現LCPrice",
    ...
    def trail(self):  # ポジションのトレールを設定（LCの底上げとは別に考える）
        if (
            self.crcdo_self_trail_exe and self.crcdo_history
        ):  # 一回すでにCDCRO実施済みが前提。
            p = self.position  # ポジションの情報
            o = self.order  # オーダーの情報
            if (
                p["time_past"] - self.crcdo_sec_counter > 10
            ):  # 前回のCRCDOよりN秒以上空いていれば、CRCDOを再検討する
                if (
                    p["pips"] > 0.02
                ):  # 勝ちpipsがＮ以上の場合、利確底上げ（トレール）を行う
                    temp_lc_range = (
                        float(p["pips"]) * self.crcdo_trail_ratio
                    )  # 含み益の７割（３割戻り）までを目標の戻り幅とする
                    temp_lc_price = round(
                        p["price"] + (temp_lc_range * self.plan["ask_bid"]), 3
                    )  # 仮のLC価格を算出する
...
def order_setting(class_order_arr, info_l):
    ...
    for i in range(len(class_order_arr)):
        ...
        lc = (
            order_info_temp["lc"] if adj_flag == 0 else lc_adj
        )  # order_info_temp['lc']  # 勝っている場合は強制LC狭く！
        ...
        target_class.crcdo_border_by_time = order_info_temp[
            "crcdo_border_by_time"
        ]  # 時間LC変更
        target_class.crcdo_max_lc_by_time = order_info_temp[
            "crcdo_max_lc_by_time"
        ]  # 時間LC変更
        target_class.crcdo_target_lc_by_time = order_info_temp[
            "crcdo_target_lc_by_time"
        ]  # 時間LC変更
        target_class.order_timeout_min = order_info_temp["order_timeout_min"]
        target_class.crcdo_self_trail_exe = order_info_temp[
            "crcdo_self_trail_exe"
        ]  # トレールは実施しない
...
def mode1():
    ...
    # 一旦整理。。
    # (1) turn2関連
    result_turn2_result = ans_dic["turn2_ans"][
        "turn_ans"
    ...
    # (3) turn3関連
    result_turn3_result = ans_dic["turn3_ans"][
        "turn_ans"
    ]  # 直近のターンがあるかどうか（連続性の考慮無し）
    ...
    if result_turn2_result == 1:  # ターンが確認された場合（最優先）
        ...
        order_mini["crcdo_trail_ratio"] = (
            0.8  # トレール時、勝ちのN%ラインにトレールする
        )
        ...
        order = {
            ...
            "lc": result_turn2_orders[
                "lc"
            ],  # 0.035,  # 非常に狭いLC(ターンミスの場合は、ストレートに下がることを期待しているため）
            ...
            "crcdo_self_trail_exe": True,  # トレールは実施しない
            "crcdo_border_by_time": lc_change_time,  # ９分(540秒）を超えたらLC短縮対象と亜する
            "crcdo_max_lc_by_time": result_turn2_orders[
                "max_lc_range"
            ],  # 最大許容のLC(短時間中の）
...
def exe_manage():
    ...
    # 時刻の分割（処理で利用）
    oanda_class.str_to_time(gl_data5r_df.iloc[0]["time_jp"])
...
def exe_loop(interval, fun, wait=True):
    ...
    time.time()
    d5_df = oa.InstrumentsCandles_multi_exe(
        "USD_JPY", {"granularity": "M5", "count": 30}, 1
    )  # 時間昇順
    if d5_df["error"] == -1:
        error_end(d5_df)
    else:
...
def all_update_information():
    ...
    """
    (
        main_c.update_information()
    )  # 露払い時の変更を取得しておく（エラーが出たら、、どうしよう）
...
gl_midnight_close_flag = (
    0  # 深夜突入時に一回だけポジション等の解消を行うフラグ　＠time_manageのみで変更あり
)
gl_exe_mode = 0  # 実行頻度のモード設定　＠
gl_total_pips = 0  # totalの合計値
gl_total_yen = 0  # Unit含めたトータル損益（円の完全）
gl_data5r_df = (
    0  # 毎回複数回ローソクを取得は時間無駄なので１回を使いまわす　＠exe_manageで取得
)
...
temp_magnification = (
    1  # 基本本番環境で動かす。unitsを低めに設定している為、ここで倍率をかけれる。
)
gl_position_num = 0
...
if fx_mode == 1:  # practice
    oa = oanda_class.Oanda(
        tk.accountID, tk.access_token, tk.environment
    )  # インスタンス生成
    gl_live = "Pra"
else:  # Live
    oa = oanda_class.Oanda(
