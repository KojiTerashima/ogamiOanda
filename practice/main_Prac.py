...
def exe_manage():
    ...
    # ■実行を行う
        ...
        gl_midnight_close_flag = (
            0  # 実行可能時には深夜フラグを解除しておく（毎回やってしまうけどいいや）
        )
        ...
        gl_now_price_mid = price_dic[
            "mid"
        ]  # 念のために保存しておく（APIの回数減らすため）
        ...
        if gl_latest_exe_time == 0:
            ...
            past_time = (
                datetime.datetime.now().replace(microsecond=0) - gl_latest_exe_time
            ).seconds

        if (
            time_min % 5 == 0 and 15 <= time_sec < 30 and past_time > 60
        ):  # キャンドルの確認　秒指定だと飛ぶので、前回から●秒経過&秒数に余裕を追加
            ...
            gl_data5r_df = d5_df.sort_index(
                ascending=False
            )  # 対象となるデータフレーム（直近が上の方にある＝時間降順）をグローバルに
            d5_df.to_csv(
                tk.folder_path + "main_data5.csv", index=False, encoding="utf-8"
            )  # 直近保存用
            mode1()
            gl_latest_exe_time = datetime.datetime.now().replace(
                microsecond=0
            )  # 最終実行時間の取得
        elif (
            time_min % 1 == 0 and time_sec % 10 == 0
        ):  # 高頻度での確認事項（キャンドル調査時のみ飛ぶ）
            mode2()
