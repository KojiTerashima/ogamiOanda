...
def Inspection_main(df_r):
    ...
    peaks_info = p.make_peaks_finalize(
        df_r[:35], 10
    )  # Peaksの算出（ループ時間短縮の為、必要最低限のピーク数（＝４）を指定する）
    ...
    line_result = ri.find_lines(
        df_r[:35]
    )  # Lineが発見された場合には、['line_strength']が１以上になる
    latest = line_result["latest_line"]
    latest_line = (
        line_result["latest_line"]["line_price"]
        if line_result["latest_line"]["line_strength"] != 0
        else 0
    )
    upper_line = (
        line_result["upper_info"]["line_price"] if line_result["found_upper"] else 0
    )
    lower_line = (
        line_result["lower_info"]["line_price"] if line_result["found_lower"] else 0
    )
    ...
    if latest["line_strength"] != 0:
        ...
        if line_result["found_upper"] and line_result["found_lower"]:
            ...
            now_upper_gap = (
                upper_line - now_price
            )  # プラス値が現在価格が範囲内。マイナス値の場合は現在価格がUpper超え
            (
                now_price - lower_line
            )  # プラス値が現在価格が範囲内。マイナス値の場合は現在価格がlower以下
            # UpperもLowerもある場合、二つの間のGapを求める
            line_gap = round(
                line_result["upper_info"]["line_price"]
                - line_result["lower_info"]["line_price"],
                3,
            )
            ...
            if line_result["found_upper"]:
                ...
                if now_price > upper_line:
                    ...
                    comment_now = (
                        "Upperのみ（現在価格がUpperより下。再度Upperに近づく）"
                    )
                    upper_over_order = {
                        "target": now_price + 0.01,
                        "expected_direction": 1,
                ...
                if now_price < lower_line:
                    ...
                    comment_now = (
                        "Lowerのみ(現在価格がLowerより上。再度Lowerに近づく方向へ"
                    )
                    lower_in_order = {
                        "target": now_price + 0.01,
                        "expected_direction": -1,
