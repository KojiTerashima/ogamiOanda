...
def get_data(main_path):
    ...
    print("検証用データ")
    print(
        "検証時間の総取得期間は", start_s5_time, "-", end_s5_time, len(gl_s5_df), "行"
    )
    ...
    d5_df_for_merge = gl_d5_df.rename(
        columns=lambda x: f"{x}_y"
    )  # 結合する側の名前にあらかじめ＿ｙをつけておく(予期せぬtime_xができるため）
    gl_inspection_base_df = pd.merge(
        gl_s5_df, d5_df_for_merge, left_on="time_jp", right_on="time_jp_y", how="left"
    )
    ...
    gl_inspection_base_df = gl_inspection_base_df.reset_index(drop=True)
    # 実際の調査開始時間と終了時間を取得する
    gl_inspection_base_df.iloc[0]["time_jp"]
    gl_inspection_base_df.iloc[-1]["time_jp"]
